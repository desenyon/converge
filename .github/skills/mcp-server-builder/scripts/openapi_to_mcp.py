...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input", help="OpenAPI file path (JSON or YAML). If omitted, reads from stdin."
    )
    parser.add_argument("--server-name", required=True, help="MCP server name.")
    parser.add_argument(
        "--language", choices=["python", "typescript"], default="python", help="Scaffold language."
    )
...
def parse_openapi(raw: str) -> dict[str, Any]:
    try:
        ...
        try:
            ...
            raise CLIError(
                "Input is not valid JSON and PyYAML is unavailable for YAML parsing."
            ) from exc
        except Exception as exc:
...
def write_outputs(
    server_name: str, language: str, output_dir: Path, tools: list[dict[str, Any]]
) -> GenerationSummary:
