...
class DebtDashboard:
    ...
    def __init__(self, team_size: int = 5):
        ...
        self.thresholds = {"excellent": 85, "good": 70, "fair": 55, "poor": 40}
        ...
        for file_path in file_paths:
            try:
                ...
                if isinstance(data, dict) and "debt_items" in data:
                    ...
                    snapshot = {
                        "file_path": file_path,
                        "scan_date": data.get("scan_metadata", {}).get(
                    ...
                    snapshot = {
                        "file_path": file_path,
                        "scan_date": data.get("metadata", {}).get(
                            "analysis_date", self._extract_date_from_filename(file_path)
                        ),
    ...
    def _process_snapshots(self):
        ...
        for snapshot in self.historical_data:
            processed = {
                ...
                "debt_by_type": Counter(
                    item.get("type", "unknown") for item in snapshot["debt_items"]
                ),
                "debt_by_severity": Counter(
                    item.get("severity", "medium") for item in snapshot["debt_items"]
                ),
                "debt_by_category": Counter(
                    self._categorize_debt_item(item) for item in snapshot["debt_items"]
                ),
                "total_files": snapshot["summary"].get(
                    "total_files_scanned", len(snapshot["file_statistics"])
                ),
                "total_effort_estimate": self._calculate_total_effort(snapshot["debt_items"]),
                "high_priority_count": len(
                    [item for item in snapshot["debt_items"] if self._is_high_priority(item)]
                ),
                "security_debt_count": len(
                    [item for item in snapshot["debt_items"] if self._is_security_related(item)]
                ),
                "raw_data": snapshot,
            }
    ...
    def _is_high_priority(self, item: dict[str, Any]) -> bool:
        ...
        return (
            ...
            or priority_score >= 7
            or debt_type in ["security_risk", "syntax_error", "architecture_debt"]
        )
    ...
    def _is_security_related(self, item: dict[str, Any]) -> bool:
        ...
        return debt_type in security_types or any(
    ...
    def _calculate_health_metrics(self):
        ...
        for snapshot in self.processed_snapshots:
            ...
            complex_debt_ratio = (
                snapshot["debt_by_type"].get("high_complexity", 0)
                + snapshot["debt_by_type"].get("large_function", 0)
            ) / max(1, snapshot["total_debt_items"])
            complexity_score = max(0, 100 - (complex_debt_ratio * 100))

            # Test coverage proxy (based on test debt)
            test_debt_ratio = snapshot["debt_by_category"].get("testing", 0) / max(
                1, snapshot["total_debt_items"]
            )
            test_coverage_proxy = max(0, 100 - (test_debt_ratio * 150))

            # Documentation proxy (based on documentation debt)
            doc_debt_ratio = snapshot["debt_by_category"].get("documentation", 0) / max(
                1, snapshot["total_debt_items"]
            )
            documentation_proxy = max(0, 100 - (doc_debt_ratio * 100))

            # Security score (based on security debt)
            security_debt_ratio = snapshot["security_debt_count"] / max(
                1, snapshot["total_debt_items"]
            )
            security_score = max(0, 100 - (security_debt_ratio * 200))

            # Maintainability (based on architecture and code quality debt)
            maint_debt_count = snapshot["debt_by_category"].get("architecture", 0) + snapshot[
                "debt_by_category"
            ].get("code_quality", 0)
            ...
            high_impact_ratio = snapshot["high_priority_count"] / max(
                1, snapshot["total_debt_items"]
            )
            velocity_impact = min(50, high_impact_ratio * 30 + debt_density * 5)

            # Technical risk (0-100, higher is more risky)
            risk_factors = snapshot["security_debt_count"] + snapshot["debt_by_type"].get(
                "architecture_debt", 0
            )
    ...
    def _calculate_trend(
        self, values: list[float], dates: list[datetime], metric_name: str
    ) -> TrendAnalysis:
        """Calculate trend analysis for a specific metric."""
        if len(values) < 2:
            return TrendAnalysis(
                metric_name, "stable", 0.0, 0.0, values[-1], (values[-1], values[-1])
            )
            ...
            try:
                correlation = numerator / (
                    (
                        ...
                        * sum((values[i] - y_mean) ** 2 for i in range(n))
                    )
                    ** 0.5
                )
    ...
    def _generate_forecasts(self) -> dict[str, Any]:
        ...
        if health_trend:
            ...
            forecasts["health_score_3_months"] = max(
                0, min(100, current_score + (health_trend.change_rate * 3))
            )
            forecasts["health_score_6_months"] = max(
                0, min(100, current_score + (health_trend.change_rate * 6))
            )

        # Debt accumulation forecast
        if self.velocity_history:
            avg_net_change = mean(
                [v.net_change for v in self.velocity_history[-3:]]
            )  # Last 3 periods
        ...
        if risk_trend:
            ...
            forecasts["risk_score_3_months"] = max(
                0, min(100, current_risk + (risk_trend.change_rate * 3))
            )
    ...
    def _generate_executive_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_debt_items": self.processed_snapshots[-1]["total_debt_items"]
            if self.processed_snapshots
            else 0,
            "estimated_effort_hours": self.processed_snapshots[-1]["total_effort_estimate"]
            if self.processed_snapshots
            else 0,
            "high_priority_items": self.processed_snapshots[-1]["high_priority_count"]
            if self.processed_snapshots
            else 0,
    ...
    def _generate_strategic_recommendations(self) -> list[dict[str, Any]]:
        ...
        if self.velocity_history:
            recent_velocities = (
                self.velocity_history[-3:]
                if len(self.velocity_history) >= 3
                else self.velocity_history
            )
    ...
    def _generate_visualization_data(self) -> dict[str, Any]:
        ...
        visualizations["effort_trend"] = [
            {"date": snapshot["date"][:10], "total_effort": snapshot["total_effort_estimate"]}
            for snapshot in self.processed_snapshots
        ]
    ...
    def _get_detailed_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_effort_per_item": current["total_effort_estimate"]
            / max(1, current["total_debt_items"]),
        }
...
def format_dashboard_report(dashboard_data: dict[str, Any]) -> str:
    ...
    if metadata["date_range"]["start"]:
        output.append(
            f"Date Range: {metadata['date_range']['start'][:10]} to {metadata['date_range']['end'][:10]}"
        )
    output.append("")
    ...
    if trends:
        ...
        for metric, trend in trends.items():
            direction_symbol = {"improving": "↑", "declining": "↓", "stable": "→"}.get(
                trend["trend_direction"], "→"
            )

            output.append(
                f"{metric.replace('_', ' ').title()}: {direction_symbol} {trend['trend_direction']}"
            )
