...
class DebtPrioritizer:
    ...
    def __init__(self, team_size: int = 5, sprint_capacity_hours: int = 80):
        ...
        self.framework_weights = {
            ...
            "rice": {"reach": 0.25, "impact": 0.25, "confidence": 0.25, "effort": 0.25},
        }
    ...
    def analyze_and_prioritize(self, framework: str = "cost_of_delay") -> dict[str, Any]:
        ...
        self.prioritized_items = sorted(
            enriched_items, key=lambda x: x["priority_score"], reverse=True
        )
    ...
    def _assess_business_impact(self, item: dict[str, Any]) -> BusinessImpact:
        ...
        # Adjust by severity
        severity_adjustments = {"low": 0.6, "medium": 1.0, "high": 1.4, "critical": 1.8}

        adjustment = severity_adjustments.get(severity, 1.0)

        # Apply adjustment and cap at 10
        adjusted_impacts = [min(10, max(1, round(impact * adjustment))) for impact in base_impacts]
    ...
    def _calculate_interest_rate(
        self, item: dict[str, Any], business_impact: BusinessImpact
    ) -> InterestRate:
    ...
    def _calculate_cost_of_delay(
        self, interest_rate: InterestRate, effort: EffortEstimate
    ) -> float:
        """Calculate total cost of delay if debt is not fixed."""

        # Estimate delay in days (assuming debt gets fixed eventually)
        estimated_delay_days = effort.hours_estimate / (
            self.sprint_capacity_hours / 14
        )  # 2-week sprints

        # Calculate cumulative cost
        daily_cost = (
            interest_rate.daily_cost
    ...
    def _calculate_cost_of_delay_score(self, item: dict[str, Any]) -> float:
        ...
        # Risk reduction
        risk_reduction = (
            business_impact["security_impact"] * 0.6 + business_impact["quality_impact"] * 0.4
        )
    ...
    def _calculate_wsjf_score(self, item: dict[str, Any]) -> float:
        ...
        # Business value
        business_value = (
            business_impact["customer_impact"] * 0.4 + business_impact["revenue_impact"] * 0.6
        )
        ...
        # Risk reduction
        risk_reduction = (
            business_impact["security_impact"] * 0.5 + business_impact["quality_impact"] * 0.5
        )
    ...
    def _generate_sprint_allocation(self) -> dict[str, Any]:
        ...
        total_effort_hours = sum(
            item["effort_estimate"]["hours_estimate"] for item in self.prioritized_items
        )
        ...
        for item in self.prioritized_items:
            ...
            if current_sprint["total_hours"] + item_effort <= debt_capacity_per_sprint:
                ...
                current_sprint["capacity_used"] = (
                    current_sprint["total_hours"] / debt_capacity_per_sprint
                )
            else:
                # Start new sprint
    ...
    def _generate_insights(self) -> dict[str, Any]:
        ...
        # Risk analysis
        high_risk_items = [
            item for item in self.prioritized_items if item["effort_estimate"]["risk_factor"] >= 1.5
        ]
        ...
        quick_wins = [
            item
            for item in self.prioritized_items
            if (item["effort_estimate"]["hours_estimate"] <= 8 and item["priority_score"] >= 3)
        ]

        # Cost analysis
        total_cost_of_delay = sum(item["cost_of_delay"] for item in self.prioritized_items)
        avg_interest_rate = sum(
            item["interest_rate"]["daily_cost"] for item in self.prioritized_items
        ) / len(self.prioritized_items)
        ...
        return {
            ...
            "top_categories_by_effort": sorted(
                effort_by_category.items(), key=lambda x: x[1], reverse=True
            )[:3],
        }
    ...
    def _generate_charts_data(self) -> dict[str, Any]:
        ...
        pie_data = [{"category": k, "effort": round(v, 1)} for k, v in effort_by_category.items()]
...
def main():
    ...
    parser.add_argument(
        "--framework",
        choices=["cost_of_delay", "wsjf", "rice"],
        default="cost_of_delay",
        help="Prioritization framework",
    )
    parser.add_argument("--team-size", type=int, default=5, help="Team size")
    parser.add_argument("--sprint-capacity", type=int, default=80, help="Sprint capacity in hours")
