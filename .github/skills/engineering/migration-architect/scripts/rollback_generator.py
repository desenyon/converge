...
class RollbackTriggerCondition:
    ...
    name: str
    condition: str
    metric_threshold: dict[str, Any] | None
...
""",
    ...
    def _generate_rollback_phases(self, migration_plan: dict[str, Any]) -> list[RollbackPhase]:
        ...
        for i, phase in enumerate(reversed(migration_phases)):
            ...
            rollback_phase = RollbackPhase(
                ...
                communication_requirements=self._get_communication_requirements(
                    phase_name, phase_risk
                ),
    ...
    def _generate_rollback_steps(
        self, phase_name: str, migration_type: str, phase_index: int
    ) -> list[RollbackStep]:
                ...
                steps.extend(
                    [
                        RollbackStep(
                            ...
                            dependencies=[],
                            validation_commands=[
                                "SELECT COUNT(*) FROM pg_stat_activity WHERE query LIKE '%migration%';"
                            ],
                        ...
                        RollbackStep(
                            ...
                            script_type="bash",
                            script_content=templates.get("data_rollback", {}).get(
                                "restore_backup", "pg_restore -d {database_name} -c {backup_file}"
                            ),
                ...
                steps.extend(
                    [
                        RollbackStep(
                            ...
                            script_type="bash",
                            script_content=templates.get("deployment_rollback", {}).get(
                                "update_load_balancer",
                                "aws elbv2 modify-rule --rule-arn {rule_arn} --actions Type=forward,TargetGroupArn={original_target_group}",
                            ),
                            estimated_duration_minutes=2,
                            dependencies=[],
                            validation_commands=["curl -f {health_check_url}"],
                            success_criteria=[
                                "Traffic routing to original service",
                        ...
                        RollbackStep(
                            ...
                            script_content=templates.get("deployment_rollback", {}).get(
                                ...
                                "kubectl rollout undo deployment/{service_name} --to-revision={revision_number}",
                            ),
            ...
            steps.extend(
                [
                    RollbackStep(
                        ...
                        script_type="bash",
                        script_content=templates.get("cloud_rollback", {}).get(
                            "revert_terraform",
                            "terraform apply -target={resource_name} {rollback_plan_file}",
                        ),
                        estimated_duration_minutes=15,
                        dependencies=[],
                        validation_commands=["terraform plan -detailed-exitcode"],
                        success_criteria=[
                            "Infrastructure matches previous state",
                            "No planned changes",
                        ],
                    ...
                    RollbackStep(
                        ...
                        script_content=templates.get("cloud_rollback", {}).get(
                            "restore_dns",
                            "aws route53 change-resource-record-sets --hosted-zone-id {zone_id} --change-batch file://{rollback_dns_changes}",
                        ),
                        estimated_duration_minutes=10,
    ...
    def _generate_communication_templates(
        self, migration_plan: dict[str, Any]
...
""",
    ...
    def _get_validation_checkpoints(self, phase_name: str, migration_type: str) -> list[str]:
        ...
        validation_commands = self.validation_templates.get(migration_type, [])
        checkpoints.extend(
            [f"Validation command passed: {cmd[:50]}..." for cmd in validation_commands[:3]]
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate comprehensive rollback runbooks from migration plans"
    )
    ...
    try:
        ...
        if "migration_id" not in migration_plan and "source" not in migration_plan:
            print(
                "Error: Migration plan must contain migration_id or source field", file=sys.stderr
            )
            return 1
