...
class SchemaCompatibilityChecker:
    ...
    def _build_type_compatibility_matrix(self) -> dict[str, dict[str, str]]:
        ...
        return {
            ...
            "boolean": {"tinyint": "compatible", "varchar": "breaking", "int": "breaking"},
    ...
    def analyze_database_schema(
        self, before_schema: dict[str, Any], after_schema: dict[str, Any]
    ) -> CompatibilityReport:
            ...
            if table_name not in before_tables:
                migration_scripts.append(
                    MigrationScript(
                        ...
                        script_content=self._generate_create_table_sql(
                ...
                for method in before_paths[path]:
                    issues.append(
                        CompatibilityIssue(
                            ...
                            suggested_migration="Implement redirect to replacement endpoint or maintain backward compatibility stub",
    ...
    def _analyze_table_changes(
        self, table_name: str, before_table: dict[str, Any], after_table: dict[str, Any]
    ) -> tuple[list[CompatibilityIssue], list[MigrationScript]]:
        ...
        for col_name in after_columns:
            if col_name not in before_columns:
                ...
                is_required = (
                    col_def.get("nullable", True) == False and col_def.get("default") is None
                )
                ...
                if is_required:
                    issues.append(
                        CompatibilityIssue(
                            ...
                            suggested_migration="Add default value or make column nullable initially",
    ...
    def _analyze_column_changes(
        self, table_name: str, col_name: str, before_col: dict[str, Any], after_col: dict[str, Any]
    ) -> tuple[list[CompatibilityIssue], list[MigrationScript]]:
        ...
        if before_type != after_type:
            compatibility = self.type_compatibility_matrix.get(before_type, {}).get(
                after_type, "breaking"
            )
            ...
            if compatibility == "breaking":
                issues.append(
                    CompatibilityIssue(
                        ...
                        suggested_migration="Add conversion logic and validate data integrity",
                ...
                issues.append(
                    CompatibilityIssue(
                        ...
                        suggested_migration="Validate all existing data can be converted safely",
            ...
            if before_nullable and not after_nullable:  # null -> not null
                issues.append(
                    CompatibilityIssue(
                        ...
                        suggested_migration="Update NULL values to valid defaults before applying NOT NULL constraint",
            ...
            if after_length < before_length:
                issues.append(
                    CompatibilityIssue(
                        ...
                        suggested_migration="Validate no existing data exceeds new length limit",
                        affected_operations=["INSERT", "UPDATE"],
                    )
                )
    ...
    def _analyze_constraint_changes(
        self, table_name: str, before_constraints: dict[str, Any], after_constraints: dict[str, Any]
    ) -> tuple[list[CompatibilityIssue], list[MigrationScript]]:
        ...
        for constraint_type in ["primary_key", "foreign_key", "unique", "check"]:
            ...
            before_set = (
                set(str(c) for c in before_constraint)
                if isinstance(before_constraint, list)
                else {str(before_constraint)}
                if before_constraint
                else set()
            )
            after_set = (
                set(str(c) for c in after_constraint)
                if isinstance(after_constraint, list)
                else {str(after_constraint)}
                if after_constraint
                else set()
            )
            ...
            for constraint in before_set - after_set:
                implication = self.constraint_implications.get(constraint_type, {}).get(
                    "removed", "non_breaking"
                )
                issues.append(
                    CompatibilityIssue(
                        ...
                        suggested_migration="Consider application-level validation for removed constraint",
    ...
    def _analyze_endpoint_changes(
        self, path: str, before_endpoint: dict[str, Any], after_endpoint: dict[str, Any]
    ) -> tuple[list[CompatibilityIssue], list[MigrationScript]]:
        ...
        for method in set(before_endpoint.keys()) & set(after_endpoint.keys()):
            ...
            for status_code in before_responses:
                if status_code in after_responses:
                    before_schema = (
                        before_responses[status_code]
                        .get("content", {})
                        .get("application/json", {})
                        .get("schema", {})
                    )
                    after_schema = (
                        after_responses[status_code]
                        .get("content", {})
                        .get("application/json", {})
                        .get("schema", {})
                    )
    ...
    def _analyze_model_changes(
        self, model_name: str, before_model: dict[str, Any], after_model: dict[str, Any]
    ) -> tuple[list[CompatibilityIssue], list[MigrationScript]]:
    ...
    def _build_compatibility_report(
        ...
        potentially_breaking_count = sum(
            1 for issue in issues if issue.severity == "potentially_breaking"
        )
        ...
        risk_assessment = {
            ...
            "deployment_risk": "requires_coordinated_deployment"
            if breaking_count > 0
            else "safe_independent_deployment",
            ...
            + (
                ...
                if any(s.script_type == "sql" for s in migration_scripts)
                else []
            ),
        }
        ...
        return CompatibilityReport(
            schema_before=json.dumps(before_schema, indent=2)[:500] + "..."
            if len(json.dumps(before_schema)) > 500
            else json.dumps(before_schema, indent=2),
            schema_after=json.dumps(after_schema, indent=2)[:500] + "..."
            if len(json.dumps(after_schema)) > 500
            else json.dumps(after_schema, indent=2),
            ...
            if severity in issues_by_severity:
                ...
                for issue in issues_by_severity[severity]:
                    ...
                    if issue.affected_operations:
                        output.append(
                            f"  Affected Operations: {', '.join(issue.affected_operations)}"
                        )
                    output.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze schema and API compatibility between versions"
    )
    ...
    parser.add_argument(
        "--type", choices=["database", "api"], default="database", help="Schema type to analyze"
    )
    parser.add_argument("--output", "-o", help="Output file for compatibility report (JSON)")
    parser.add_argument(
        "--format", "-f", choices=["json", "text", "both"], default="both", help="Output format"
    )
