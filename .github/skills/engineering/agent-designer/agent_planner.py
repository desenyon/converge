...
class AgentPlanner:
    ...
    def _define_common_tools(self) -> dict[str, Tool]:
        ...
        return {
            ...
            "code_executor": Tool(
                ...
                output_schema={
                    "type": "object",
                    "properties": {"result": {"type": "string"}, "error": {"type": "string"}},
                },
            ...
            "file_manager": Tool(
                ...
                output_schema={
                    "type": "object",
                    "properties": {"success": {"type": "boolean"}, "content": {"type": "string"}},
                },
            ...
            "data_analyzer": Tool(
                ...
                input_schema={
                    "type": "object",
                    "properties": {"data": {"type": "object"}, "analysis_type": {"type": "string"}},
                },
                output_schema={
                    "type": "object",
                    "properties": {"insights": {"type": "array"}, "metrics": {"type": "object"}},
                },
            ...
            "api_client": Tool(
                ...
                output_schema={
                    "type": "object",
                    "properties": {"response": {"type": "object"}, "status": {"type": "integer"}},
                },
    ...
    def select_architecture_pattern(
        ...
        for pattern, heuristics in self.pattern_heuristics.items():
            ...
            if pattern == AgentArchitecturePattern.SINGLE_AGENT and complexity_score <= 2:
                ...
                pattern == AgentArchitecturePattern.PIPELINE
                and "sequential" in requirements.description.lower()
            ):
                score += 3
            elif (
                pattern == AgentArchitecturePattern.SWARM
                and "parallel" in requirements.description.lower()
            ):
                score += 3
            ...
            if performance_reqs.get("high_throughput", False) and pattern in [
                AgentArchitecturePattern.SWARM,
                AgentArchitecturePattern.PIPELINE,
            ]:
                score += 2
            if (
                performance_reqs.get("fault_tolerance", False)
                and pattern == AgentArchitecturePattern.SWARM
            ):
                score += 2
            if performance_reqs.get("low_latency", False) and pattern in [
                AgentArchitecturePattern.SINGLE_AGENT,
                AgentArchitecturePattern.PIPELINE,
            ]:
                score += 1
    ...
    def design_agents(
        self, requirements: SystemRequirements, pattern: AgentArchitecturePattern
    ...
    def _design_supervisor_agents(self, requirements: SystemRequirements) -> list[AgentDefinition]:
        ...
        supervisor = AgentDefinition(
            ...
            constraints={"max_concurrent_supervisions": 5, "decision_timeout": "30s"},
            success_criteria=[
        ...
        for i, domain in enumerate(task_domains[: requirements.team_size - 1]):
            specialist = AgentDefinition(
                ...
                constraints={"domain_scope": domain, "task_queue_size": 10},
                success_criteria=[
                    f"excel in {domain} tasks",
    ...
    def _design_hierarchical_agents(
        self, requirements: SystemRequirements
    ) -> list[AgentDefinition]:
    ...
    def _design_pipeline_agents(self, requirements: SystemRequirements) -> list[AgentDefinition]:
        ...
        for i, stage in enumerate(pipeline_stages):
            agent = AgentDefinition(
                ...
                constraints={"processing_order": i + 1, "batch_size": 10, "stage_timeout": "5min"},
                success_criteria=[
                    f"successfully process {stage}",
    ...
    def design_communication_topology(
        self, agents: list[AgentDefinition], pattern: AgentArchitecturePattern
    ...
    def generate_implementation_roadmap(
        self, design: ArchitectureDesign, requirements: SystemRequirements
    ) -> dict[str, Any]:
    ...
    def plan_system(
        self, requirements: SystemRequirements
    ) -> tuple[ArchitectureDesign, str, dict[str, Any]]:
...
def main():
    ...
    parser.add_argument(
        "--format", choices=["json", "yaml", "both"], default="both", help="Output format"
    )
