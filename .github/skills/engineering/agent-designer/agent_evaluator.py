...
class OptimizationRecommendation:
    ...
    estimated_performance_gain: float | None
...
class AgentEvaluator:
    ...
    def calculate_performance_metrics(self, logs: list[ExecutionLog]) -> PerformanceMetrics:
            ...
            if start_time and end_time:
                try:
                    ...
                    throughput_tasks_per_hour = (
                        total_tasks / time_diff_hours if time_diff_hours > 0 else 0.0
                    )
                except:
                    throughput_tasks_per_hour = 0.0
            else:
        ...
        error_rate = (
            sum(1 for log in logs if log.error_details) / total_tasks if total_tasks > 0 else 0.0
        )
        retry_rate = (
            sum(1 for log in logs if log.retry_count > 0) / total_tasks if total_tasks > 0 else 0.0
        )
        ...
        if unclassified_errors:
            ...
            error_analysis = ErrorAnalysis(
                ...
                common_patterns=self._extract_common_patterns(
                    [str(e["error"]) for e in unclassified_errors]
                ),
                suggested_fixes=[
                    "Review and classify error patterns",
    ...
    def identify_bottlenecks(
        self, logs: list[ExecutionLog], agent_metrics: dict[str, PerformanceMetrics]
            ...
            if metrics.average_duration_ms > 30000:  # 30 seconds
                ...
                bottlenecks.append(
                    BottleneckAnalysis(
                        ...
                        estimated_improvement={
                            "latency_reduction": min(
                                0.5,
                                (metrics.average_duration_ms - 10000) / metrics.average_duration_ms,
                            ),
                            "throughput_gain": 1.3,
                        },
                    )
            ...
            if usage_stats.get("error_rate", 0) > 0.2:
                bottlenecks.append(
                    BottleneckAnalysis(
                        ...
                        impact_on_performance={
                            "reliability_impact": usage_stats["error_rate"]
        ...
        if communication_analysis.get("high_latency_communications", 0) > 5:
            bottlenecks.append(
                BottleneckAnalysis(
                    ...
                    impact_on_performance={
                        "communication_overhead": communication_analysis.get(
                            "avg_communication_latency", 0
                        ),
        ...
        if resource_analysis.get("high_token_usage_tasks", 0) > 10:
            bottlenecks.append(
                BottleneckAnalysis(
                    ...
                    estimated_improvement={"cost_reduction": 0.3, "efficiency_gain": 1.1},
                )
            )
        ...
        bottlenecks.sort(
            key=lambda x: (severity_order[x.severity], -sum(x.impact_on_performance.values()))
        )
    ...
    def _analyze_communication_patterns(self, logs: list[ExecutionLog]) -> dict[str, Any]:
        ...
        avg_latency = sum(action["duration"] for action in communication_actions) / len(
            communication_actions
        )
            ...
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    expected_impact={
                        "latency_reduction": min(
                            0.5,
                            (system_metrics.average_duration_ms - 5000)
                            / system_metrics.average_duration_ms,
                        ),
            ...
            for error_analysis in high_impact_errors[:3]:  # Top 3 high impact errors
                recommendations.append(
                    OptimizationRecommendation(
                        ...
                        estimated_cost_savings=system_metrics.total_cost_usd
                        * (error_analysis.percentage / 100)
                        * 0.5,
        ...
        for bottleneck in critical_bottlenecks[:2]:  # Top 2 critical bottlenecks
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    estimated_performance_gain=list(bottleneck.estimated_improvement.values())[0]
                    if bottleneck.estimated_improvement
                    else 1.1,
        ...
        if system_metrics.throughput_tasks_per_hour < 20:
            recommendations.append(
                OptimizationRecommendation(
                    ...
                    implementation_effort="high",
                    expected_impact={"throughput_improvement": 2.0, "scalability_headroom": 5.0},
    ...
    def generate_report(self, logs: list[ExecutionLog]) -> EvaluationReport:
        ...
        summary = {
            "evaluation_period": {
                ...
                "total_duration_hours": system_metrics.total_tasks
                / system_metrics.throughput_tasks_per_hour
                if system_metrics.throughput_tasks_per_hour > 0
                else 0,
            },
    ...
    def _generate_cost_breakdown(
        ...
        return {
            ...
            "top_cost_drivers": sorted(task_type_costs.items(), key=lambda x: x[1], reverse=True)[
                :5
            ],
        }
    ...
    def _check_sla_compliance(self, metrics: PerformanceMetrics) -> dict[str, Any]:
        ...
        return {
            ...
            "compliance_score": sum(1 for sla in compliance.values() if sla["compliant"])
            / len(compliance),
        }
...
def main():
    ...
    parser.add_argument("--format", choices=["json", "both"], default="both", help="Output format")
    parser.add_argument(
    ...
    try:
        ...
        if args.format == "both":
            ...
            recommendations_data = {
                "optimization_recommendations": [
                    asdict(rec) for rec in report.optimization_recommendations
                ],
            ...
            error_data = {
                ...
                "error_summary": {
                    ...
                    "high_impact_errors": len(
                        [e for e in report.error_analysis if e.impact_level == "high"]
                    ),
                },
            }
        ...
        print(f"\n{'=' * 60}")
        print("AGENT SYSTEM EVALUATION REPORT")
        print(f"{'=' * 60}")
        ...
        print("\nTop Recommendations:")
        high_priority_recs = [
            r for r in report.optimization_recommendations if r.priority == "high"
        ][:3]
