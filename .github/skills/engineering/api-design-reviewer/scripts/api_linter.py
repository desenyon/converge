...
class LintReport:
    ...
    def calculate_score(self) -> float:
        ...
        penalty_per_endpoint = (
            total_penalty / self.total_endpoints if self.total_endpoints > 0 else total_penalty
        )
...
class APILinter:
    ...
    def _validate_path_structure(self, path: str) -> bool:
        ...
        for i, segment in enumerate(segments):
            ...
            if segment.startswith("{") and segment.endswith("}"):
                ...
                if not self.camel_case_pattern.match(
                    param_name
                ) and not self.kebab_case_pattern.match(param_name):
                    self.report.add_issue(
                        LintIssue(
                            severity="warning",
                            category="naming",
    ...
    def _validate_http_method_usage(
        ...
        if method in ["POST", "PUT", "PATCH"] and "requestBody" not in operation:
            ...
            if not any(
                action in path.lower() for action in ["activate", "deactivate", "reset", "confirm"]
            ):
                self.report.add_issue(
                    LintIssue(
                        severity="info",
    ...
    def _validate_responses(self, path: str, method: str, responses: dict[str, Any]) -> bool:
        ...
        has_error_responses = any(
            code.startswith("4") or code.startswith("5") for code in responses.keys()
        )
    ...
    def _validate_parameters(
        self, path: str, method: str, parameters: list[dict[str, Any]]
    ) -> bool:
...
Examples:
  ...
  python api_linter.py --raw-endpoints endpoints.json
    ...
    parser.add_argument(
        "input_file", help="Input file: OpenAPI/Swagger JSON file or raw endpoints JSON"
    )
