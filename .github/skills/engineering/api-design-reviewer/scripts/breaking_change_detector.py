...
class ComparisonReport:
    ...
    def calculate_summary(self) -> None:
        ...
        self.summary = {
            ...
            "potentially_breaking_changes": len(
                [c for c in self.changes if c.change_type == ChangeType.POTENTIALLY_BREAKING]
            ),
            "non_breaking_changes": len(
                [c for c in self.changes if c.change_type == ChangeType.NON_BREAKING]
            ),
            "enhancements": len(
                [c for c in self.changes if c.change_type == ChangeType.ENHANCEMENT]
            ),
            "critical_severity": len(
                [c for c in self.changes if c.severity == ChangeSeverity.CRITICAL]
            ),
            "high_severity": len([c for c in self.changes if c.severity == ChangeSeverity.HIGH]),
            "medium_severity": len(
                [c for c in self.changes if c.severity == ChangeSeverity.MEDIUM]
            ),
    ...
    def has_breaking_changes(self) -> bool:
        ...
        return any(
            c.change_type in [ChangeType.BREAKING, ChangeType.POTENTIALLY_BREAKING]
...
class BreakingChangeDetector:
            ...
            for method in old_operations:
                self.report.add_change(
                    Change(
                        ...
                        new_value=None,
                        migration_guide=self._generate_endpoint_removal_migration(
                            path, method, new_paths
                        ),
    ...
    def _compare_path_operations(self, path: str, old_path_obj: dict, new_path_obj: dict) -> None:
        ...
        for method in removed_ops:
            self.report.add_change(
                Change(
                    ...
                    old_value=f"{method.upper()} {path}",
                    new_value=None,
                    migration_guide=self._generate_method_removal_migration(
                        path, method, new_operations
                    ),
        ...
        for method in common_ops:
            self._compare_operation_details(
                path, method, old_path_obj[method], new_path_obj[method]
            )

    def _compare_operation_details(
        ...
        self._compare_parameters(
            operation_path, old_op.get("parameters", []), new_op.get("parameters", [])
        )

        # Compare request body
        self._compare_request_body(
            operation_path, old_op.get("requestBody"), new_op.get("requestBody")
        )

        # Compare responses
        self._compare_responses(
            operation_path, old_op.get("responses", {}), new_op.get("responses", {})
        )

        # Compare security requirements
        self._compare_security_requirements(
            operation_path, old_op.get("security"), new_op.get("security")
        )

    def _compare_parameters(
        self, base_path: str, old_params: list[dict], new_params: list[dict]
    ) -> None:
    ...
    def _compare_parameter_details(
        self, base_path: str, name: str, location: str, old_param: dict, new_param: dict
    ) -> None:
    ...
    def _compare_request_body(
        self, base_path: str, old_body: dict | None, new_body: dict | None
    ) -> None:
    ...
    def _compare_responses(self, base_path: str, old_responses: dict, new_responses: dict) -> None:
        ...
        for code in common_codes:
            self._compare_response_details(
                responses_path, code, old_responses[code], new_responses[code]
            )

    def _compare_response_details(
        self, base_path: str, status_code: str, old_response: dict, new_response: dict
    ) -> None:
        ...
        self._compare_content_types(
            response_path, old_content, new_content, f"response {status_code}"
        )

    def _compare_content_types(
        self, base_path: str, old_content: dict, new_content: dict, context: str
    ) -> None:
        ...
        for content_type in common_types:
            ...
            if old_schema != new_schema:
                ...
                self._compare_schemas(
                    schema_path, old_schema, new_schema, f"{context} ({content_type})"
                )

    def _compare_schemas(
        self, base_path: str, old_schema: dict, new_schema: dict, context: str
    ) -> None:
        ...
        if old_schema.get("type") == "array" and new_schema.get("type") == "array":
            ...
            if old_items != new_items:
                self._compare_schemas(
                    f"{base_path}/items", old_items, new_items, f"{context} items"
                )
    ...
    def _compare_security_requirements(
        self, base_path: str, old_security: list | None, new_security: list | None
    ) -> None:
        """Compare security requirements."""
        ...
        if old_security != new_security:
            ...
            if old_security is None and new_security is not None:
                ...
                migration_guide = (
                    "Ensure proper authentication/authorization when calling this endpoint"
                )
    ...
    def _generate_endpoint_removal_migration(
        self, removed_path: str, method: str, remaining_paths: dict[str, Any]
    ) -> str:
    ...
    def _generate_method_removal_migration(
        self, path: str, removed_method: str, remaining_methods: set[str]
    ) -> str:
    ...
    def generate_text_report(self) -> str:
        ...
        if not self.report.changes:
            ...
            breaking_changes = [
                c for c in self.report.changes if c.change_type == ChangeType.BREAKING
            ]
            potentially_breaking = [
                c for c in self.report.changes if c.change_type == ChangeType.POTENTIALLY_BREAKING
            ]
            non_breaking = [
                c for c in self.report.changes if c.change_type == ChangeType.NON_BREAKING
            ]
            enhancements = [
                c for c in self.report.changes if c.change_type == ChangeType.ENHANCEMENT
            ]
