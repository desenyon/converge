...
class ChangelogGenerator:
        ...
        for line in lines:
            ...
            if full_match:
                ...
                if current_commit:
                    ...
                    if commit_message:
                        current_commit = ConventionalCommit(
                            commit_message,
    ...
    def parse_json_commits(self, json_data: str | list[dict]):
    ...
    def generate_release_summary(self) -> dict:
        ...
        return {
            ...
            "issue_references": len(
                set().union(*(commit.extract_issue_references() for commit in self.commits))
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "--version", "-v", type=str, default="Unreleased", help="Version for this release"
    )
    parser.add_argument(
        "--date",
        "-d",
    ...
    parser.add_argument("--base-url", "-u", type=str, default="", help="Base URL for commit links")
    parser.add_argument(
        "--input-format", choices=["git-log", "json"], default="git-log", help="Input format"
    )
    parser.add_argument("--output", "-o", type=str, help="Output file (default: stdout)")
    parser.add_argument(
        "--summary", "-s", action="store_true", help="Include release summary statistics"
    )
