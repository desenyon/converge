...
class Version:
    ...
    def bump(self, bump_type: BumpType, prerelease_type: PreReleaseType | None = None) -> "Version":
        """Create new version with specified bump."""
        if bump_type == BumpType.NONE:
            return Version(
                self.major, self.minor, self.patch, self.prerelease_type, self.prerelease_number
            )
...
class ConventionalCommit:
    ...
    @classmethod
    def parse_message(
        cls, message: str, commit_hash: str = "", author: str = "", date: str = ""
    ) -> "ConventionalCommit":
...
class VersionBumper:
    ...
    def parse_commits_from_json(self, json_data: str | list[dict]):
    ...
    def recommend_version(self, prerelease_type: PreReleaseType | None = None) -> Version:
    ...
    def generate_bump_commands(self, new_version: Version) -> dict[str, list[str]]:
        ...
        commands = {
            ...
            "rust": ["# Update Cargo.toml", "# [package]", f"# version = '{version_str}'"],
            "git": [
        ...
        for commit in self.commits:
            ...
            if commit.is_breaking:
                ...
                analysis["features"].append(
                    {"scope": commit.scope, "description": commit.description, "hash": commit.hash}
                )
            elif commit.type in ["fix", "security", "perf", "bugfix"]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Determine version bump based on conventional commits"
    )
    parser.add_argument(
        "--current-version", "-c", required=True, help="Current version (e.g., 1.2.3, v1.2.3)"
    )
    parser.add_argument("--input", "-i", type=str, help="Input file with commits (default: stdin)")
    parser.add_argument(
        "--input-format", choices=["git-log", "json"], default="git-log", help="Input format"
    )
    parser.add_argument(
        "--prerelease", "-p", choices=["alpha", "beta", "rc"], help="Generate pre-release version"
    )
    parser.add_argument(
        "--output-format",
        "-f",
    ...
    parser.add_argument(
        "--include-commands", action="store_true", help="Include bump commands in output"
    )
    parser.add_argument("--include-files", action="store_true", help="Include file update snippets")
    parser.add_argument("--custom-rules", type=str, help="JSON string with custom type->bump rules")
    parser.add_argument("--ignore-types", type=str, help="Comma-separated list of types to ignore")
    parser.add_argument(
        "--analysis", "-a", action="store_true", help="Include detailed commit analysis"
    )
