...
class Feature:
    ...
    test_coverage_actual: float | None = None
...
class ReleasePlanner:
    ...
    def load_release_plan(self, plan_data: str | dict):
        ...
        for feature_data in data.get("features", []):
            try:
                ...
                print(
                    f"Warning: Error parsing feature {feature_data.get('id', 'unknown')}: {e}",
                    file=sys.stderr,
                )
        ...
        for gate_data in data.get("quality_gates", []):
            try:
                ...
                print(
                    f"Warning: Error parsing quality gate {gate_data.get('name', 'unknown')}: {e}",
                    file=sys.stderr,
                )
    ...
    def assess_release_readiness(self) -> dict:
        ...
        for feature in self.features:
            ...
            # Check test coverage
            if (
                feature.test_coverage_actual is not None
                and feature.test_coverage_actual < feature.test_coverage_required
            ):
        ...
        for gate in self.quality_gates:
            ...
            if gate.status == ComponentStatus.READY:
                ...
                if gate.required:
                    assessment["warnings"].append(f"Required quality gate '{gate.name}' is pending")
        ...
        if self.target_date:
            ...
            now = (
                datetime.now(self.target_date.tzinfo) if self.target_date.tzinfo else datetime.now()
            )
            ...
            if days_until_release < 0:
                assessment["blocking_issues"].append(
                    f"Release is {abs(days_until_release)} days overdue"
                )
        ...
        if feature_stats["high_risk"] > 0:
            assessment["recommendations"].append(
                "Review high-risk features for additional validation"
            )
    ...
    def generate_release_checklist(self) -> list[dict]:
            ...
            [
                {
                    ...
                    "status": "ready"
                    if all(f.status == ComponentStatus.READY for f in self.features)
                    else "pending",
        ...
        for item, status in approval_items:
            checklist.append(
                {"category": "Approvals", "item": item, "status": "ready" if status else "pending"}
            )
        ...
        for item in deployment_items:
            checklist.append({"category": "Deployment", "item": item, "status": "pending"})
    ...
    def _check_security_approvals(self) -> bool:
        ...
        high_risk_features = [
            f for f in self.features if f.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]
        ]
    ...
    def generate_communication_plan(self) -> dict:
        ...
        internal_stakeholders = [
            ...
            if s.role in ["developer", "qa", "pm", "devops", "security"]
        ]
        external_stakeholders = [
            s for s in self.stakeholders if s.role in ["customer", "partner", "support"]
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Assess release readiness and generate release plans"
    )
    parser.add_argument("--input", "-i", required=True, help="Release plan JSON file")
    parser.add_argument(
        "--output-format",
        "-f",
    ...
    parser.add_argument(
        "--include-checklist", action="store_true", help="Include release checklist in output"
    )
    parser.add_argument(
        "--include-communication", action="store_true", help="Include communication plan"
    )
    parser.add_argument("--include-rollback", action="store_true", help="Include rollback runbook")
    parser.add_argument(
        "--min-coverage", type=float, default=80.0, help="Minimum test coverage threshold"
    )
    ...
    if args.output_format == "json":
        ...
        if checklist:
            output_lines.extend(["## Release Checklist", ""])
            current_category = ""
