...
class UpgradePlanner:
    ...
    def analyze_upgrades(
        self, dependency_inventory: str, timeline_days: int = 90
    ) -> dict[str, Any]:
        ...
        analysis_results["upgrade_plans"] = self._create_upgrade_plans(
            analysis_results["available_upgrades"], timeline_days
        )
    ...
    def _analyze_dependency_upgrade(self, dependency: dict[str, Any]) -> DependencyUpgrade | None:
        ...
        # Assess upgrade risk
        risk_level = self._assess_upgrade_risk(
            name, current_ver, latest_ver, ecosystem, update_type
        )
    ...
    def _parse_version(self, version_string: str) -> VersionInfo | None:
        ...
        if match:
            ...
            return VersionInfo(major=int(major), minor=int(minor), patch=int(patch or 0))
    ...
    def _get_latest_version(self, package_name: str, ecosystem: str) -> str | None:
    ...
    def _has_known_breaking_changes(
        self, package_name: str, current: VersionInfo, latest: VersionInfo
    ) -> bool:
    ...
    def _check_security_updates(
        ...
        if package_name in self.security_advisories:
            for advisory in self.security_advisories[package_name]:
                ...
                ) and not self._is_version_greater(fixed_version, latest_version):
    ...
    def _analyze_breaking_changes(
        self, package_name: str, current: VersionInfo, latest: VersionInfo, ecosystem: str
    ) -> list[str]:
        ...
        if latest.major > current.major:
            breaking_changes.append(
                f"Major version upgrade from {current.major}.x to {latest.major}.x"
            )
    ...
    def _estimate_migration_effort(
        self, update_type: UpdateType, breaking_changes: list[str]
    ) -> str:
    ...
    def _get_affected_dependencies(
        self, package_name: str, dependency: dict[str, Any]
    ) -> list[str]:
    ...
    def _perform_risk_assessment(self, upgrades: list[DependencyUpgrade]) -> dict[str, Any]:
        ...
        high_risk_upgrades = [
            u for u in upgrades if u.risk_level in [UpgradeRisk.HIGH, UpgradeRisk.CRITICAL]
        ]
    ...
    def _identify_risk_factors(self, upgrades: list[DependencyUpgrade]) -> list[str]:
        ...
        framework_upgrades = [
            u
            for u in upgrades
            if any(fw in u.name.lower() for fw in ["react", "django", "spring", "webpack", "babel"])
        ]
    ...
    def _create_upgrade_plans(
        self, upgrades: list[DependencyUpgrade], timeline_days: int
    ) -> list[UpgradePlan]:
        """Create phased upgrade plans."""
        if not upgrades:
            return []
        ...
        phase1_upgrades = [
            u for u in sorted_upgrades if u.security_updates or u.risk_level == UpgradeRisk.SAFE
        ][:10]
        if phase1_upgrades:
            plans.append(
                self._create_upgrade_plan(
        ...
        phase2_upgrades = [
            u
            for u in sorted_upgrades
            if u.risk_level in [UpgradeRisk.LOW, UpgradeRisk.MEDIUM] and not u.security_updates
        ][:8]
        if phase2_upgrades:
            plans.append(
                self._create_upgrade_plan(
        ...
        phase3_upgrades = [
            u for u in sorted_upgrades if u.risk_level in [UpgradeRisk.HIGH, UpgradeRisk.CRITICAL]
        ][:5]
        if phase3_upgrades:
            plans.append(
                self._create_upgrade_plan(
    ...
    def _generate_upgrade_recommendations(self, analysis_results: dict[str, Any]) -> list[str]:
        ...
        if security_count > 0:
            recommendations.append(
                f"URGENT: {security_count} security updates available - prioritize immediately"
            )

        safe_count = analysis_results["upgrade_statistics"]["by_risk"].get("safe", 0)
        if safe_count > 0:
            recommendations.append(
                f"Quick wins: {safe_count} safe updates can be applied with minimal risk"
            )

        critical_count = analysis_results["risk_assessment"]["high_risk_count"]
        if critical_count > 0:
            recommendations.append(
                f"Plan carefully: {critical_count} high-risk upgrades need thorough testing"
            )
    ...
    def generate_report(self, analysis_results: dict[str, Any], format: str = "text") -> str:
        ...
        if format == "json":
            ...
            serializable_results["available_upgrades"] = [
                asdict(upgrade) for upgrade in analysis_results["available_upgrades"]
            ]
            serializable_results["upgrade_plans"] = [
                asdict(plan) for plan in analysis_results["upgrade_plans"]
            ]
        ...
        high_priority = sorted(
            [u for u in analysis_results["available_upgrades"]],
        ...
        if high_priority:
            ...
            for upgrade in high_priority:
                risk_indicator = (
                    "🔴"
                    if upgrade.risk_level in [UpgradeRisk.HIGH, UpgradeRisk.CRITICAL]
                    else "🟡"
                    if upgrade.risk_level == UpgradeRisk.MEDIUM
                    else "🟢"
                )
                security_indicator = " 🔒" if upgrade.security_updates else ""

                report.append(
                    f"{risk_indicator} {upgrade.name}: {upgrade.current_version} → {upgrade.latest_version}{security_indicator}"
                )
                report.append(
                    f"   Type: {upgrade.update_type.value.title()} | Risk: {upgrade.risk_level.value.title()} | Priority: {upgrade.priority_score:.1f}"
                )
...
Examples:
  ...
  python upgrade_planner.py deps.json --risk-threshold medium --output plan.txt
    ...
    parser.add_argument("inventory_file", help="Path to dependency inventory JSON file")
    parser.add_argument(
        "--timeline", type=int, default=90, help="Timeline for upgrade plan in days (default: 90)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--security-only", action="store_true", help="Only plan upgrades with security fixes"
    )
    ...
    try:
        ...
        if args.risk_threshold != "critical":
            ...
            results["available_upgrades"] = [
                u for u in results["available_upgrades"] if u.risk_level.value in allowed_risks
            ]

        # Filter for security-only if specified
        if args.security_only:
            results["available_upgrades"] = [
                u for u in results["available_upgrades"] if u.security_updates
            ]
