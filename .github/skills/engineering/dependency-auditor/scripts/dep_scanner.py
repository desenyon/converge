...
class DependencyScanner:
        ...
        for pkg_name in package_names:
            if pkg_name in self.known_vulnerabilities:
                for vuln in self.known_vulnerabilities[pkg_name]:
                    if self._version_matches_vulnerability(
                        dependency.version, vuln.affected_versions
                    ):
    ...
    def _version_matches_vulnerability(self, version: str, affected_pattern: str) -> bool:
        ...
        try:
            ...
            if "<" in affected_pattern and ">" not in affected_pattern:
                ...
                return (
                    self._compare_versions(version, min_part) >= 0
                    and self._compare_versions(version, max_part) < 0
                )
        except:
            pass
    ...
    def _parse_yarn_lock(self, file_path: Path) -> list[Dependency]:
        ...
        try:
            ...
            for package_spec, version in packages:
                ...
                dep = Dependency(name=name, version=version, ecosystem="npm", direct=False)
                dependencies.append(dep)
            ...
            for line in lines:
                ...
                if line and not line.startswith("#") and not line.startswith("-"):
                    ...
                    if match:
                        ...
                        dep = Dependency(name=name, version=version, ecosystem="pypi", direct=True)
                        dependencies.append(dep)
    ...
    def _parse_poetry_lock(self, file_path: Path) -> list[Dependency]:
        ...
        try:
            ...
            for name, version in packages:
                dep = Dependency(name=name, version=version, ecosystem="pypi", direct=False)
                dependencies.append(dep)
        ...
        try:
            ...
            if require_match:
                ...
                for line in requires.split("\n"):
                    ...
                    if match:
                        name, version = match.groups()
                        dep = Dependency(name=name, version=version, ecosystem="go", direct=True)
                        dependencies.append(dep)
        ...
        try:
            ...
            if dep_section:
                for line in dep_section.group(1).split("\n"):
                    ...
                    if match:
                        name, version = match.groups()
                        dep = Dependency(name=name, version=version, ecosystem="cargo", direct=True)
                        dependencies.append(dep)
    ...
    def _parse_cargo_lock(self, file_path: Path) -> list[Dependency]:
        ...
        try:
            ...
            for name, version in packages:
                dep = Dependency(name=name, version=version, ecosystem="cargo", direct=False)
                dependencies.append(dep)
    ...
    def _parse_gemfile(self, file_path: Path) -> list[Dependency]:
        ...
        try:
            ...
            for gem_info in gems:
                ...
                dep = Dependency(name=name, version=version, ecosystem="rubygems", direct=True)
                dependencies.append(dep)
        ...
        try:
            ...
            if gem_section:
                ...
                for name, version in gems:
                    dep = Dependency(name=name, version=version, ecosystem="rubygems", direct=False)
                    dependencies.append(dep)
    ...
    def _generate_scan_summary(self, scan_results: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "vulnerable_dependencies": len(
                [dep for dep in scan_results["dependencies"] if dep.vulnerabilities]
            ),
    ...
    def _generate_recommendations(self, scan_results: dict[str, Any]) -> list[str]:
        ...
        if high_count > 0:
            recommendations.append(
                f"URGENT: Address {high_count} high-severity vulnerabilities immediately"
            )

        if medium_count > 0:
            recommendations.append(
                f"Schedule fixes for {medium_count} medium-severity vulnerabilities within 30 days"
            )
            ...
            for dep in vulnerable_deps[:3]:  # Top 3 most critical
                for vuln in dep.vulnerabilities:
                    if vuln.fixed_version:
                        recommendations.append(
                            f"Update {dep.name} from {dep.version} to {vuln.fixed_version} to fix {vuln.id}"
                        )
