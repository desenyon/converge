...
class SelectivityEstimator:
    ...
    def __init__(self):
        ...
        self.very_low_selectivity_patterns = [r"is_.*", r"has_.*", r"can_.*", r"boolean", r"bool"]
...
class IndexOptimizer:
    ...
    def analyze_missing_indexes(self) -> list[IndexRecommendation]:
        ...
        for pattern in self.query_patterns:
            ...
            where_columns = [
                cond.get("column")
                for cond in pattern.where_conditions
    ...
    def _create_single_column_recommendation(
        self, table_name: str, column: str, pattern: QueryPattern, operator: str
    ...
    def _create_composite_recommendation(
        self, table_name: str, columns: list[str], pattern: QueryPattern
    ) -> IndexRecommendation | None:
        ...
        if len(index_name) > 63:  # PostgreSQL limit
            index_name = (
                f"idx_{table_name}_composite_{abs(hash('_'.join(ordered_columns))) % 10000}"
            )

        index = Index(
            name=index_name, table=table_name, columns=ordered_columns, index_type="btree"
        )
        ...
        return IndexRecommendation(
            ...
            priority=self._calculate_priority(
                combined_selectivity, pattern.frequency, len(ordered_columns)
            ),
    ...
    def _create_covering_index_recommendation(
        self, table_name: str, where_columns: list[str], pattern: QueryPattern
    ) -> IndexRecommendation | None:
        """Create recommendation for covering index."""
        order_columns = [
            col["column"] for col in pattern.order_by if col["column"] in self.tables[table_name]
        ]
        ...
        return IndexRecommendation(
            recommendation_id=self._generate_recommendation_id(
                table_name, index_columns, "covering"
            ),
            table=table_name,
            recommended_index=index,
            reason=reason,
            ...
            priority=self._calculate_priority(
                main_selectivity, pattern.frequency, len(index_columns)
            ),
    ...
    def _create_join_index_recommendation(
        self, table_name: str, column: str, pattern: QueryPattern, join_condition: dict[str, Any]
    ) -> IndexRecommendation | None:
        ...
        index = Index(name=index_name, table=table_name, columns=[column], index_type="btree")
        ...
        return IndexRecommendation(
            ...
            sql_statement=f"CREATE INDEX {index_name} ON {table_name} ({column});",
            selectivity_analysis={"column_selectivity": selectivity, "join_optimization": True},
        )
        ...
        for rec in recommendations:
            ...
            if index_signature not in seen_indexes:
                ...
                for existing_rec in unique_recommendations:
                    if (
                        existing_rec.table == rec.table
                        and existing_rec.recommended_index.columns == rec.recommended_index.columns
                    ):
    ...
    def _find_duplicate_indexes(
        self, table_name: str, indexes: list[Index]
    ) -> list[RedundancyIssue]:
            ...
            if not index_columns.intersection(used_columns):
                issues.append(
                    RedundancyIssue(
                        ...
                        sql_statements=[
                            f"-- Review usage before dropping\n-- DROP INDEX {index.name};"
                        ],
                    )
                )
    ...
    def estimate_index_sizes(self) -> dict[str, dict[str, Any]]:
        ...
        for table_name in self.tables:
            size_estimates[table_name] = {
                ...
                "existing_indexes_size_mb": len(self.existing_indexes.get(table_name, []))
                * 5,  # Rough estimate
    ...
    def generate_analysis_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "sql_statements": {
                ...
                "drop_redundant": [
                    stmt for issue in redundancy_issues for stmt in issue.sql_statements
                ],
            },
            "performance_impact": self._generate_performance_impact_analysis(recommendations),
        }

    def _generate_performance_impact_analysis(
        self, recommendations: list[IndexRecommendation]
    ) -> dict[str, Any]:
        """Generate performance impact analysis."""
        impact_analysis = {"query_optimization": {}, "write_overhead": {}, "storage_impact": {}}
        ...
        impact_analysis["query_optimization"] = {
            ...
            "high_impact_queries": len(
                [
                    q
                    for q, benefits in query_benefits.items()
                    if any("High" in benefit for benefit in benefits)
                ]
            ),
            "benefit_distribution": dict(Counter(rec.estimated_benefit for rec in recommendations)),
        }
        ...
        impact_analysis["write_overhead"] = {
            ...
            "tables_most_affected": list(
                Counter(rec.table for rec in recommendations).most_common(3)
            ),
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize database indexes based on schema and query patterns"
    )
    ...
    parser.add_argument(
        "--analyze-existing", "-e", action="store_true", help="Include analysis of existing indexes"
    )
    parser.add_argument(
        "--min-priority",
        "-p",
        ...
        if args.min_priority < 4:
            for priority_level in ["high_priority", "medium_priority", "low_priority"]:
                analysis["index_recommendations"][priority_level] = [
                    rec
                    for rec in analysis["index_recommendations"][priority_level]
