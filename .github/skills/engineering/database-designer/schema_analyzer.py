...
class SchemaAnalyzer:
    ...
    def _parse_column_definition(self, definition: str) -> Column | None:
        """Parse individual column definition."""
        # Pattern for column definition
        pattern = re.compile(r"(\w+)\s+([A-Z]+(?:\(\d+(?:,\d+)?\))?)\s*(.*)", re.IGNORECASE)
    ...
    def _parse_foreign_key(self, definition: str) -> tuple[str, str] | None:
    ...
    def _parse_unique_constraint(self, definition: str) -> list[str] | None:
    ...
    def _parse_check_constraint(self, definition: str) -> dict[str, str] | None:
    ...
    def _parse_indexes(self, ddl_content: str) -> None:
        ...
        index_pattern = re.compile(
            r"CREATE\s+(?:(UNIQUE)\s+)?INDEX\s+(\w+)\s+ON\s+(\w+)\s*\(\s*(.*?)\s*\)", re.IGNORECASE
        )
        ...
        for match in index_pattern.finditer(ddl_content):
            ...
            index = Index(name=index_name, table=table_name, columns=columns, unique=unique)
        ...
        for column in table.columns:
            if any(pattern in column.data_type.upper() for pattern in ["ARRAY", "JSON", "TEXT"]):
                if "JSON" in column.data_type.upper():
                    ...
                    self.normalization_issues.append(
                        NormalizationIssue(
                            table=table.name,
            ...
            if column.data_type.upper().startswith(("VARCHAR", "CHAR", "TEXT")):
                if any(
                    delimiter in column.name.lower()
                    for delimiter in ["list", "array", "tags", "items"]
                ):
                    self.normalization_issues.append(
                        NormalizationIssue(
                            table=table.name,
    ...
    def _check_size_optimization(self, table_name: str, column: Column) -> None:
        ...
        if column.data_type.upper() == "BIGINT":
            if not any(
                keyword in column.name.lower() for keyword in ["timestamp", "big", "large", "count"]
            ):
                self.datatype_issues.append(
                    DataTypeIssue(
                        table=table_name,
        ...
        return {
            "schema_overview": {
                ...
                "total_foreign_keys": sum(
                    len(table.foreign_keys) for table in self.tables.values()
                ),
            ...
            "normalization_analysis": {
                ...
                "by_severity": {
                    ...
                    "warning": len(
                        [i for i in self.normalization_issues if i.severity == "WARNING"]
                    ),
                },
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        high_severity_issues = [
            i for i in self.normalization_issues + self.constraint_issues if i.severity == "HIGH"
        ]

        if high_severity_issues:
            recommendations.append(
                f"Address {len(high_severity_issues)} high-severity issues immediately"
            )
        ...
        if varchar_255_issues:
            recommendations.append(
                f"Review {len(varchar_255_issues)} VARCHAR(255) columns for right-sizing"
            )
        ...
        if missing_fks:
            recommendations.append(
                f"Consider adding {len(missing_fks)} foreign key constraints for referential integrity"
            )
        ...
        if normalization_issues_count > 0:
            recommendations.append(
                f"Review {normalization_issues_count} normalization issues for schema optimization"
            )
    ...
    def format_text_report(self, analysis: dict[str, Any]) -> str:
        ...
        if dt_analysis["total_issues"] > 0:
            ...
            for issue in dt_analysis["issues"][:5]:  # Show first 5
                ...
                lines.append(
                    f"  Current: {issue['current_type']} → Suggested: {issue['suggested_type']}"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze database schema for design issues and generate ERD"
    )
    parser.add_argument("--input", "-i", required=True, help="Input file (SQL DDL or JSON schema)")
    parser.add_argument("--output", "-o", help="Output file (default: stdout)")
    parser.add_argument(
        "--output-format", "-f", choices=["json", "text"], default="text", help="Output format"
    )
    parser.add_argument(
        "--generate-erd", "-e", action="store_true", help="Include Mermaid ERD in output"
    )
