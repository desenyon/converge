...
class SLODesigner:
    ...
    def create_service_definition(
        self, service_type: str, criticality: str, user_facing: bool, name: str = None
    ) -> dict[str, Any]:
    ...
    def generate_slis(self, service_def: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        base_slis = self.SERVICE_TYPE_SLIS.get(
            service_type, ["availability", "latency", "error_rate"]
        )
    ...
    def generate_slos(
        self, service_def: dict[str, Any], slis: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def _create_slo_from_sli(
        self, sli: dict[str, Any], targets: dict[str, float], service_def: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def calculate_error_budgets(self, slos: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for slo in slos:
            if slo["operator"] == ">=":  # Availability-type SLOs
                ...
                time_windows = {"1h": 3600, "1d": 86400, "7d": 604800, "30d": 2592000}
    ...
    def _generate_burn_rate_alerts(
        self, slo: dict[str, Any], error_budget_rate: float
    ) -> list[dict[str, Any]]:
        ...
        for window_config in self.BURN_RATE_WINDOWS:
            alert = {
                ...
                "severity": self._determine_alert_severity(
                    float(window_config["budget_consumed"].rstrip("%"))
                ),
    ...
    def generate_sla_recommendations(
        self, service_def: dict[str, Any], slos: list[dict[str, Any]]
    ) -> dict[str, Any]:
    ...
    def _generate_penalty_structure(self, criticality: str) -> list[dict[str, Any]]:
        ...
        penalty_structures = {
            ...
            "medium": [{"breach_threshold": "< 99.5%", "credit_percentage": 10}],
            "low": [],
        }
    ...
    def _generate_implementation_guide(
        self, service_def: dict[str, Any], slis: list[dict[str, Any]], slos: list[dict[str, Any]]
    ) -> dict[str, Any]:
...
Examples:
    ...
    parser.add_argument("--input", "-i", help="Input service definition JSON file")
    parser.add_argument("--output", "-o", help="Output framework JSON file")
    parser.add_argument(
        "--service-type",
    ...
    parser.add_argument(
        "--user-facing", choices=["true", "false"], help="Whether service is user-facing"
    )
    parser.add_argument("--service-name", help="Service name")
    parser.add_argument(
        "--summary-only", action="store_true", help="Only display summary, do not save JSON"
    )
    ...
    if not args.input and not (args.service_type and args.criticality and args.user_facing):
        parser.error(
            "Must provide either --input file or --service-type, --criticality, and --user-facing"
        )
