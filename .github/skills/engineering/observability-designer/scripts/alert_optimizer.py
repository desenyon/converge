...
class AlertOptimizer:
    ...
    SEVERITY_PRIORITY = {"critical": 1, "high": 2, "warning": 3, "info": 4}
        ...
        for alert in alerts:
            ...
            if historical_data:
                ...
                if false_positive_rate > 0.3:  # > 30% false positives
                    ...
                    noise_reasons.append(
                        f"High false positive rate: {false_positive_rate * 100:.1f}%"
                    )
            ...
            if noise_score >= 3:  # Threshold for considering an alert noisy
                noisy_alert = {
                    ...
                    "recommendations": self._generate_noise_reduction_recommendations(
                        alert, noise_reasons
                    ),
                }
    ...
    def _generate_noise_reduction_recommendations(
        self, alert: dict[str, Any], reasons: list[str]
    ) -> list[str]:
    ...
    def identify_coverage_gaps(
        self, alerts: list[dict[str, Any]], services: list[dict[str, Any]] = None
    ) -> dict[str, Any]:
    ...
    def _classify_alert_category(self, rule: str, alert_name: str) -> str:
        ...
        if any(
            keyword in rule_lower or keyword in name_lower
    ...
    def _analyze_service_coverage(
        self, alerts: list[dict[str, Any]], services: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        ...
        for service in services:
            ...
            service_alerts = [
                alert
                for alert in alerts
                if service_name in alert.get("expr", "")
                or service_name in alert.get("labels", {}).get("service", "")
            ]
    ...
    def _generate_coverage_recommendations(self, coverage_analysis: dict[str, Any]) -> list[str]:
        ...
        for missing_category in coverage_analysis["missing_categories"]:
            if missing_category == "availability":
                ...
                recommendations.append(
                    "Add security monitoring (auth failures, suspicious activity)"
                )
        ...
        for signature, alert_group in alert_signatures.items():
            if len(alert_group) > 1:
                duplicate_group = {
                    ...
                    "signature": signature,
                    "alerts": [
                        {"index": i, "name": alert.get("alert", alert.get("name", f"Alert_{i}"))}
    ...
    def _generate_alert_signature(self, alert: dict[str, Any]) -> str:
        ...
        key_labels = {k: v for k, v in labels.items() if k in ["service", "severity", "team"]}
    ...
    def _identify_similar_alerts(
        self, alert_group: list[tuple[int, dict[str, Any]]]
    ) -> list[dict[str, Any]]:
        ...
        for alert in alerts:
            ...
            if percentage_match:
                ...
                if operator in [">", ">="] and "error" in expr.lower():
                    ...
                    analysis["recommendations"].append(
                        "Consider increasing error rate threshold based on SLO"
                    )
            ...
            if any(resource in expr.lower() for resource in ["cpu", "memory", "disk"]):
                ...
                if threshold_value and threshold_value.replace(".", "").isdigit():
                    ...
                    if threshold_num < 0.7:  # Less than 70%
                        ...
                        analysis["recommendations"].append(
                            "Consider increasing threshold to reduce noise"
                        )
            ...
            if historical_data:
                ...
                if false_positive_rate > 0.2:
                    analysis["threshold_issues"].append(
                        f"High false positive rate: {false_positive_rate * 100:.1f}%"
                    )
                    analysis["recommendations"].append(
                        "Analyze historical data and adjust threshold"
                    )
    ...
    def assess_alert_fatigue_risk(self, alerts: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if total_high_priority > 50:
            ...
            fatigue_assessment["recommendations"].append(
                "Review and reduce number of high-priority alerts"
            )
        ...
        if critical_count > 0 and warning_count > 0:
            ...
            if critical_ratio > 0.3:  # More than 30% critical
                ...
                fatigue_assessment["recommendations"].append(
                    "Review critical alert criteria - not everything should be critical"
                )
        ...
        alerts_without_runbooks = [
            alert for alert in alerts if not alert.get("annotations", {}).get("runbook_url")
        ]
        ...
        if runbook_ratio > 0.5:
            ...
            fatigue_assessment["recommendations"].append(
                "Create runbooks for alerts to improve response efficiency"
            )
    ...
    def generate_optimized_config(
        self, alerts: list[dict[str, Any]], analysis_results: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for i, alert in enumerate(alerts):
            ...
            for threshold_issue in threshold_issues:
                if threshold_issue["alert_name"] == alert_name:
                    optimized_alert = self._apply_threshold_optimization(
                        optimized_alert, threshold_issue
                    )
                    break
        ...
        if "duplicate_alerts" in analysis_results:
            optimized_alerts = self._remove_duplicate_alerts(
                optimized_alerts, analysis_results["duplicate_alerts"]
            )
    ...
    def _apply_noise_reduction(
        self, alert: dict[str, Any], noise_analysis: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for recommendation in noise_analysis["recommendations"]:
            if "for:" in recommendation and not alert.get("for"):
                ...
                optimized_alert["annotations"]["optimization_note"] = (
                    "Review threshold - potentially too sensitive"
                )

        return optimized_alert

    def _apply_threshold_optimization(
        self, alert: dict[str, Any], threshold_analysis: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if threshold_analysis["recommendations"]:
            if "annotations" not in optimized_alert:
                optimized_alert["annotations"] = {}
            optimized_alert["annotations"]["threshold_recommendations"] = "; ".join(
    ...
    def _ensure_alert_metadata(self, alert: dict[str, Any]) -> dict[str, Any]:
        ...
        if "description" not in optimized_alert["annotations"]:
            optimized_alert["annotations"]["description"] = (
                "This alert requires a description. Please update with specific details about the condition and impact."
            )
    ...
    def _remove_duplicate_alerts(
        self, alerts: list[dict[str, Any]], duplicates: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    def analyze_configuration(self, alert_config: dict[str, Any]) -> dict[str, Any]:
        ...
        analysis_results["overall_recommendations"] = self._generate_overall_recommendations(
            analysis_results
        )
    ...
    def _generate_overall_recommendations(self, analysis_results: dict[str, Any]) -> list[str]:
        ...
        if analysis_results["alert_fatigue_assessment"]["risk_level"] == "high":
            recommendations.append(
                "HIGH PRIORITY: Address alert fatigue risk by reducing alert volume"
            )
        ...
        if len(analysis_results["noisy_alerts"]) > 0:
            recommendations.append(
                f"Optimize {len(analysis_results['noisy_alerts'])} noisy alerts to reduce false positives"
            )

        if len(analysis_results["duplicate_alerts"]) > 0:
            recommendations.append(
                f"Remove or consolidate {len(analysis_results['duplicate_alerts'])} duplicate alert groups"
            )
    ...
    def export_analysis(
        self, analysis_results: dict[str, Any], output_file: str, format_type: str = "json"
    ):
...
<body>
    ...
    <div class="section warning">
        ...
        <p><strong>Risk Level:</strong> {
            analysis_results["alert_fatigue_assessment"]["risk_level"].upper()
        }</p>
    ...
    <div class="section info">
        ...
        {
            "".join(
                f"<div><strong>{alert['alert_name']}</strong> (Score: {alert['noise_score']})<ul>{''.join(f'<li>{reason}</li>' for reason in alert['reasons'])}</ul></div>"
    ...
    <div class="section info">
        <h2>Coverage Gaps</h2>
        <p><strong>Missing Categories:</strong> {
            ", ".join(analysis_results["coverage_gaps"]["missing_categories"]) or "None"
        }</p>
        <p><strong>Missing Golden Signals:</strong> {
            ", ".join(analysis_results["coverage_gaps"]["missing_golden_signals"]) or "None"
        }</p>
        <p><strong>Critical Gaps:</strong> {
            len(analysis_results["coverage_gaps"]["critical_gaps"])
        }</p>
    </div>
...
Examples:
    ...
    parser.add_argument("--input", "-i", required=True, help="Input alert configuration JSON file")
    parser.add_argument("--output", "-o", help="Output optimized configuration JSON file")
    parser.add_argument("--report", "-r", help="Generate analysis report file")
    parser.add_argument(
        "--format", choices=["json", "html"], default="json", help="Report format (json or html)"
    )
    parser.add_argument(
        "--analyze-only",
        action="store_true",
    ...
    try:
        ...
        if not args.analyze_only:
            optimized_config = optimizer.generate_optimized_config(
                alert_config.get("alerts", alert_config.get("rules", [])), analysis_results
            )
