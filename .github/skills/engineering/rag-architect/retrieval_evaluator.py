...
class RetrievalEvaluator:
    ...
    def _identify_failure_patterns(self, query_results: list[dict[str, Any]]) -> list[str]:
        ...
        if vocab_mismatch_count > len(query_results) * 0.2:
            patterns.append(
                f"Vocabulary mismatch: {vocab_mismatch_count} queries may have vocabulary mismatch issues"
            )
        ...
        if zero_results > len(query_results) * 0.1:
            patterns.append(
                f"Query specificity: {zero_results} queries returned no results (may be too specific)"
            )
        ...
        if low_recall > len(query_results) * 0.3:
            patterns.append(
                f"Low recall: {low_recall} queries have recall@10 < 0.5 (missing relevant documents)"
            )
...
def load_queries(file_path: str) -> list[dict[str, Any]]:
    ...
    if isinstance(data, list):
        ...
        raise ValueError(
            "Invalid query file format. Expected list of queries or {'queries': [...]}."
        )
...
def load_ground_truth(file_path: str) -> dict[str, list[str]]:
    ...
    if isinstance(data, dict):
        ...
        raise ValueError(
            "Invalid ground truth format. Expected dict mapping query_id -> relevant_doc_ids."
        )
...
def generate_recommendations(evaluation_results: dict[str, Any]) -> list[str]:
    ...
    if p1 < 0.3:
        recommendations.append(
            "LOW PRECISION: Consider implementing query expansion or reranking to improve result quality."
        )

    if p5 < 0.4:
        recommendations.append(
            "RANKING ISSUES: Current ranking may not prioritize relevant documents. Consider BM25 or learning-to-rank models."
        )
    ...
    if r5 < 0.5:
        recommendations.append(
            "LOW RECALL: Consider query expansion techniques (synonyms, related terms) to find more relevant documents."
        )

    if r10 - r5 > 0.2:
        recommendations.append(
            "RANKING DEPTH: Many relevant documents found in positions 6-10. Consider increasing default result count."
        )
    ...
    if mrr < 0.4:
        recommendations.append(
            "POOR RANKING: First relevant result appears late in rankings. Implement result reranking."
        )
    ...
    if zero_results > total_queries * 0.1:
        recommendations.append(
            "COVERAGE ISSUES: Many queries return no results. Check for vocabulary mismatch or missing content."
        )
    ...
    if short_perf < 0.3:
        recommendations.append(
            "SHORT QUERY ISSUES: Brief queries perform poorly. Consider query completion or suggestion features."
        )

    if long_perf > short_perf + 0.2:
        recommendations.append(
            "QUERY PROCESSING: Longer queries perform better. Consider query parsing to extract key terms."
        )

    # General recommendations
    if not recommendations:
        recommendations.append(
            "GOOD PERFORMANCE: System performs well overall. Consider A/B testing incremental improvements."
        )
...
def main():
    ...
    parser.add_argument(
        "ground_truth", help="JSON file containing ground truth relevance judgments"
    )
    ...
    try:
        ...
        print(
            f"Loaded {len(queries)} queries, {len(documents)} documents, ground truth for {len(ground_truth)} queries"
        )
