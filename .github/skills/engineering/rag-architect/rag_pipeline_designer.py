...
class PipelineDesign:
    ...
    reranking: ComponentRecommendation | None
...
class RAGPipelineDesigner:
    ...
    def design_pipeline(self, requirements: Requirements) -> PipelineDesign:
        ...
        total_cost = (
            chunking.cost_monthly
            + embedding.cost_monthly
            + vector_db.cost_monthly
            + retrieval.cost_monthly
    ...
    def _recommend_reranking(
        self, req: Requirements, scale: Scale
...
"""
    ...
    def _generate_config_templates(self, *components) -> dict[str, Any]:
        ...
        configs["deployment"] = {
            ...
            if any("pinecone" in str(c.name) for c in components if c)
            else "hybrid",
            "scaling": {"auto_scaling": True, "min_replicas": 1, "max_replicas": 10},
            "monitoring": {
