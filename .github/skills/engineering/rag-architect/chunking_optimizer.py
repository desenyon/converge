...
class DocumentCorpus:
    ...
    def _load_documents(self):
                ...
                try:
                    ...
                    if content.strip():  # Only include non-empty files
                        self.documents.append(
                            {"path": str(file_path), "content": content, "size": len(content)}
                        )
        ...
        print(
            f"Loaded {len(self.documents)} documents totaling {sum(d['size'] for d in self.documents):,} characters"
        )
        ...
        for line in lines:
            ...
            if is_heading:
                if current_section["content"].strip():
                    sections.append(current_section)
                current_section = {"heading": heading_text, "content": "", "level": heading_level}
            else:
...
class ChunkingOptimizer:
        ...
        for doc in corpus.documents:
            try:
                ...
                document_results.append(
                    {"path": doc["path"], "chunk_count": len(chunks), "analysis": doc_analysis}
                )
            except Exception as e:
    ...
    def _calculate_performance_score(self, analysis: dict[str, Any]) -> float:
        ...
        size_consistency = (
            1.0 - min(size_stats["std"] / size_stats["mean"], 1.0) if size_stats["mean"] > 0 else 0
        )
        boundary_score = (
            boundary_quality["sentence_boundary_ratio"] + boundary_quality["word_boundary_ratio"]
        ) / 2
    ...
    def _recommend_strategy(self, results: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "reasoning": self._generate_reasoning(
                best_strategy, results[best_strategy] if best_strategy else None
            ),
        }
    ...
    def _generate_reasoning(self, strategy_name: str, result: dict[str, Any]) -> str:
        ...
        reasoning += (
            f"- Boundary quality: {boundary['sentence_boundary_ratio']:.2%} clean sentence breaks\n"
        )
        reasoning += f"- Semantic coherence: {analysis['semantic_coherence']:.3f}\n"

        return reasoning

    def _generate_sample_chunks(
        self, corpus: DocumentCorpus, strategy_name: str
    ) -> list[dict[str, Any]]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze documents and recommend optimal chunking strategy"
    )
    ...
    try:
        ...
        print(
            f"Corpus: {corpus_info['document_count']} documents, {corpus_info['total_size']:,} characters"
        )
