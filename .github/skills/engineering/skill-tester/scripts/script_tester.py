...
class ScriptTestResult:
    ...
    def add_test(self, test_name: str, passed: bool, message: str = "", details: dict = None):
        """Add a test result"""
        self.tests[test_name] = {"passed": passed, "message": message, "details": details or {}}
...
class ScriptTester:
    ...
    def _test_argparse_implementation(self, content: str, result: ScriptTestResult):
        ...
        try:
            ...
            if argparse_score == 3:
                result.add_test(
                    "argparse_implementation", True, "Complete argparse implementation found"
                )
            elif argparse_score > 0:
                result.add_test(
                    "argparse_implementation",
                    False,
                ...
                result.add_test(
                    "argparse_implementation", False, "No argparse implementation found"
                )
    ...
    def _test_script_execution(self, script_path: Path, result: ScriptTestResult):
        ...
        try:
            ...
            result.add_test(
                "basic_execution", False, f"Script execution timed out after {self.timeout} seconds"
            )
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                has_help_content = any(
                    indicator in help_output.lower() for indicator in help_indicators
                )
    ...
    def _test_sample_data_processing(self, script_path: Path, result: ScriptTestResult):
        ...
        if not assets_dir.exists():
            result.add_test(
                "sample_data_processing", True, "No sample data to test (assets dir missing)"
            )
            return
        ...
        if tested_files == 0:
            ...
            result.add_test(
                "sample_data_processing", False, "Failed to process any sample data files"
            )
    ...
    def _test_output_formats(self, script_path: Path, result: ScriptTestResult):
        ...
        try:
            ...
            if any(
                ...
                for indicator in ["json.dump", "json.load", '"json"', "--json"]
            ):
                json_support = True
            ...
            if json_support and human_readable_support:
                result.add_test(
                    "output_formats", True, "Supports both JSON and human-readable output"
                )
                ...
                result.add_test(
                    ...
                    f"Supports only {format_type} output",
                    {
                        ...
                        "human_readable_support": human_readable_support,
                    },
                )
...
Test Categories:
  ...
  - Output format compliance
    ...
    parser.add_argument("skill_path", help="Path to the skill directory containing scripts to test")
    parser.add_argument(
        "--timeout",
        type=int,
        default=30,
