...
def score_ad(issues):
    ...
    for category, items in issues.items():
        if items:
            score -= deductions.get(category, 5) * (
                1 if isinstance(items, bool) else min(len(items), 3)
            )
    return max(0, score)
...
def validate_google_rsa(ad):
    ...
    if len(headlines) < spec["headline_count_min"]:
        issues["count_too_few"].append(
            f"Need ≥{spec['headline_count_min']} headlines, got {len(headlines)}"
        )
    if len(headlines) > spec["headline_count_max"]:
        issues["count_too_many"].append(
            f"Max {spec['headline_count_max']} headlines, got {len(headlines)}"
        )
    if len(descriptions) < spec["description_count_min"]:
        issues["count_too_few"].append(
            f"Need ≥{spec['description_count_min']} descriptions, got {len(descriptions)}"
        )
    ...
    for i, h in enumerate(headlines):
        ...
        if length > spec["headline_max"]:
            issues["char_over_limit"].append(
                f"Headline {i + 1}: {length} chars (max {spec['headline_max']})"
            )
    ...
    for i, d in enumerate(descriptions):
        ...
        if length > spec["description_max"]:
            issues["char_over_limit"].append(
                f"Description {i + 1}: {length} chars (max {spec['description_max']})"
            )
        report.append(
            f"  Description {i + 1}: {status} '{d}' ({length}/{spec['description_max']} chars)"
        )
...
def validate_meta_feed(ad):
    ...
    if primary:
        ...
        report.append(
            f"  Primary text: {status} ({length}/{spec['primary_text_max']} preview chars)"
        )
        if length > spec["primary_text_max"]:
            issues["char_over_limit"].append(
                f"Primary text {length} chars exceeds {spec['primary_text_max']}-char preview"
            )
    ...
    if headline:
        ...
        if length > spec["headline_max"]:
            issues["char_over_limit"].append(
                f"Headline {length} chars (max {spec['headline_max']})"
            )
...
def validate_linkedin(ad):
    ...
    if intro:
        ...
        if length > spec["intro_text_max"]:
            issues["char_over_limit"].append(
                f"Intro text {length} chars exceeds {spec['intro_text_max']}-char preview"
            )
