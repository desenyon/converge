...
def audit_bio(profile: ProfileData) -> list:
    ...
    if not bio:
        findings.append(
            AuditFinding(
                "Bio", "CRITICAL", "No bio provided for audit", "Provide bio text with --bio flag"
            )
        )
        return findings
    ...
    if len(bio) < 30:
        ...
        findings.append(
            AuditFinding(
                "Bio",
                "WARN",
                f"Bio may be too long ({len(bio)} chars)",
    ...
    if hashtags:
        findings.append(
            AuditFinding(
                "Bio",
                "WARN",
                f"Hashtags in bio ({', '.join(hashtags)})",
    ...
    if found:
        findings.append(
            AuditFinding(
                "Bio",
                "WARN",
                f"Buzzwords detected: {', '.join(found)}",
    ...
    if bio.count("|") >= 3 or bio.count("/") >= 3:
        findings.append(
            AuditFinding(
                "Bio",
                "WARN",
                "Bio may lack focus (too many roles/identities)",
    ...
    # Social proof check
    proof_patterns = [
        r"\d+[kKmM]?\+?\s*(followers|subscribers|readers|users|customers)",
        r"(founder|ceo|cto|vp|head|director|lead)\s+(of|at|@)",
        r"(author|writer)\s+of",
        r"featured\s+in",
        r"ex-\w+",
    ]
    ...
    if has_proof:
        ...
        findings.append(
            AuditFinding(
                "Bio",
                "WARN",
                "No obvious social proof in bio",
...
def audit_activity(profile: ProfileData) -> list:
    ...
    if profile.posts_per_week <= 0:
        ...
        findings.append(
            AuditFinding(
                "Activity", "GOOD", f"Good posting cadence ({profile.posts_per_week:.0f}/week)"
            )
        )
    else:
        findings.append(
            AuditFinding(
                "Activity", "GOOD", f"High posting cadence ({profile.posts_per_week:.0f}/week)"
            )
        )
    ...
    if profile.reply_ratio > 0:
        if profile.reply_ratio < 0.2:
            ...
            findings.append(
                AuditFinding("Activity", "GOOD", f"Healthy reply ratio ({profile.reply_ratio:.0%})")
            )
    ...
    if profile.followers > 0 and profile.following > 0:
        ...
        if ratio < 0.5:
            ...
            findings.append(
                AuditFinding("Profile", "GOOD", f"Healthy follower/following ratio ({ratio:.1f}x)")
            )
...
def generate_recommendations(findings: list, profile: ProfileData) -> list:
    ...
    if profile.followers < 1000:
        recs.append(
            "📈 Growth phase: Focus 70% on replies to larger accounts, 30% on your own posts"
        )
    elif profile.followers < 10000:
        recs.append(
            "📈 Momentum phase: 2-3 threads/week + daily engagement. Start a recurring series."
        )
    else:
        recs.append(
            "📈 Scale phase: Leverage audience with cross-platform repurposing + newsletter growth"
        )
...
Examples:
  ...
  %(prog)s --handle @example --followers 50000 --posts-per-week 21 --reply-ratio 0.4 --json
    ...
    parser.add_argument(
        "--reply-ratio", type=float, default=0, help="Fraction of posts that are replies (0-1)"
    )
    parser.add_argument("--has-pinned", action="store_true", help="Has a pinned tweet")
    parser.add_argument(
        "--pinned-age-days", type=int, default=0, help="Age of pinned tweet in days"
    )
