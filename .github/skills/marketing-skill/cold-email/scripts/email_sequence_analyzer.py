...
def analyze_subject_line(subject: str) -> dict:
    ...
    if subject.startswith("Re:") or subject.startswith("Fwd:"):
        ...
        if lower.startswith("re:") or lower.startswith("fwd:"):
            warnings.append(
                "Fake Re:/Fwd: subjects feel deceptive — people have learned this trick"
            )
...
def analyze_body(body: str) -> dict:
    ...
    if word_count > 200:
        deductions.append(
            ("word_count", 15, f"Too long ({word_count} words) — cold emails should be under 150")
        )
    elif word_count > 150:
        deductions.append(
            ("word_count", 5, f"Getting long ({word_count} words) — aim for under 150")
        )
    elif word_count < 30:
        deductions.append(
            ("word_count", 10, f"Very short ({word_count} words) — may lack enough context")
        )
    ...
    if avg_wps > 25:
        deductions.append(
            (
                "readability",
                10,
                f"Sentences average {avg_wps:.0f} words — too complex, aim for 15-20",
            )
        )
    ...
    for opener in DEAD_OPENERS:
        if opener in body_lower:
            deductions.append(
                ("opener", 20, f"Dead opener detected: '{opener}' — rewrite the opening")
            )
            break
    ...
    if pers_density < 5:
        deductions.append(
            ("personalization", 10, "Low personalization signals — email may feel generic")
        )
    ...
    if triggered:
        deductions.append(
            ("spam", len(triggered) * 5, f"Spam trigger words: {', '.join(triggered[:5])}")
        )
    ...
    if not has_strong_cta:
        deductions.append(
            ("cta", 15, "No clear CTA detected — every cold email needs a single, direct ask")
        )
    ...
    if re.search(r'<html|<body|<table|<div|style="|font-family:', body_lower):
        deductions.append(
            (
                "format",
                20,
                "HTML detected — plain text emails get better deliverability for cold outreach",
            )
        )
    ...
    if len(links) > 2:
        deductions.append(
            ("links", 10, f"{len(links)} links detected — keep to 1-2 max for cold email")
        )
...
def print_report(results: list[dict]) -> None:
    ...
    for r in results:
        ...
        print(
            f"   CTA: {'✅ Clear ask detected' if body['has_strong_cta'] else '❌ No clear CTA found'}"
        )
