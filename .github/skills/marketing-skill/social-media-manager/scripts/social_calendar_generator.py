...
DEMO_CONFIG = {
    "pillars": [
        ...
        {
            "name": "Inspirational",
            "description": "Success stories & quotes",
            "emoji": "✨",
            "weight": 2,
        },
...
def build_markdown(result: dict) -> str:
    m = result["meta"]
    lines = []
    lines.append("# Social Media Content Calendar")
    lines.append(
...
def pretty_print(result: dict) -> None:
    ...
    for pname, pc in result["platforms"].items():
        print(f"\n  {'─' * 60}")
        print(f"  📣  {pname.upper()}  — {pc['total_posts']} posts  ({pc['posts_per_week']}/week)")
    ...
    for week_num in sorted(weeks_data.keys()):
        ...
        print(f"  {'Date':<12} {'Day':<11}" + "".join(f" {p:<22}" for p in m["platforms"]))
...
def parse_args():
    ...
    parser.add_argument(
        "--start", type=str, default=None, help="Start date YYYY-MM-DD (overrides config)"
    )
    parser.add_argument(
        "--weeks", type=int, default=None, help="Number of weeks to generate (overrides config)"
    )
    parser.add_argument("--json", action="store_true", help="Output calendar as JSON")
    parser.add_argument("--markdown", action="store_true", help="Output calendar as Markdown")
