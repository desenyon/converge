...
class TeamScalingCalculator:
    ...
    def calculate_scaling_plan(self, current_state: dict, growth_targets: dict) -> dict:
        ...
        results["team_structure"] = self._design_team_structure(growth_targets["target_headcount"])
...
def calculate_team_scaling(current_state: dict, growth_targets: dict) -> str:
    ...
    sorted_roles = sorted(
        results["hiring_plan"]["by_role"].items(), key=lambda x: x[1]["priority"], reverse=True
    )

    for role, details in sorted_roles[:5]:
        output.append(
            f"  {role}: {details['hires_needed']} hires (Priority: {details['priority']}/10)"
        )
...
if __name__ == "__main__":
    ...
    if args.input_file:
        ...
        growth_targets = {"target_headcount": 75, "timeline_quarters": 4}
