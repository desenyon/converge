...
class APIScorecard:
    ...
    def get_top_recommendations(self, limit: int = 5) -> list[str]:
        ...
        weighted_recs = []
        for category_score in sorted(
            self.category_scores.values(), key=lambda x: x.weight, reverse=True
        ):
...
class APIScoringEngine:
    ...
    def _extract_api_info(self) -> None:
        ...
        for path_obj in paths.values():
            if isinstance(path_obj, dict):
                endpoint_count += len(
                    [m for m in path_obj.keys() if m.upper() in self.http_methods]
                )
    ...
    def _score_consistency(self) -> None:
        ...
        score = CategoryScore(
            category=category, score=0.0, max_score=100.0, weight=self.category_weights[category]
        )
                ...
                avg_consistency = sum(
                    len(
                        [
                            code
                            for code in pattern
                            if code_usage.get(code, 0) > len(status_patterns) * 0.5
                        ]
                    )
                    for pattern in status_patterns
                ) / len(status_patterns)
    ...
    def _check_schema_documentation(self) -> float:
        ...
        for schema_name, schema in schemas.items():
            ...
            if schema.get("example") or any(
                isinstance(prop, dict) and prop.get("example") for prop in properties.values()
            ):
                doc_elements += 1
    ...
    def _check_example_coverage(self) -> float:
        ...
        for path_obj in paths.values():
            ...
            for method, operation in path_obj.items():
                ...
                for response in responses.values():
                    if isinstance(response, dict) and self._has_examples(
        ...
        # Combine scores
        operation_score = (
            (operations_with_examples / total_operations * 100) if total_operations > 0 else 100
        )
            ...
            for prop_name, prop_def in properties.items():
                ...
                if any(sensitive in prop_name.lower() for sensitive in sensitive_field_names):
                    ...
                    if not (
                        ...
                        or prop_def.get("format") == "password"
                        or "password" in prop_def.get("description", "").lower()
                    ):
    ...
    def _check_error_handling(self) -> float:
        ...
        error_detail = (
            (detailed_error_responses / operations_with_errors * 40)
            if operations_with_errors > 0
            else 0
        )
    ...
    def _check_filtering_and_searching(self) -> float:
        ...
        return (
            (endpoints_with_filtering / collection_endpoints * 100)
            if collection_endpoints > 0
            else 100
        )
    ...
    def _check_resource_relationships(self) -> float:
        ...
        nested_score = (
            (nested_resources / total_resource_paths * 50) if total_resource_paths > 0 else 25
        )
    ...
    def _check_developer_experience(self) -> float:
        ...
        operation_id_score = (
            (operations_with_ids / total_operations * 100) if total_operations > 0 else 100
        )
    ...
    def _check_pagination_patterns(self) -> float:
            ...
            if "{" not in path:  # No path parameters = collection
                ...
                if get_operation and isinstance(get_operation, dict):
                    ...
                    has_pagination = any(
                        ...
                        and param.get("name", "").lower() in pagination_params
                        for param in parameters
                    )
        ...
        return (
            (paginated_endpoints / collection_endpoints * 100) if collection_endpoints > 0 else 100
        )
    ...
    def _check_efficiency_patterns(self) -> float:
        ...
        return (
            (operations_with_selection / total_get_operations * 100)
            if total_get_operations > 0
            else 60
        )
    ...
    def generate_text_report(self) -> str:
        ...
        sorted_categories = sorted(
            self.scorecard.category_scores.items(), key=lambda x: x[1].weight, reverse=True
        )
        ...
        if top_recs:
            lines.extend(["", "🎯 TOP PRIORITY RECOMMENDATIONS:", ""])
...
Examples:
  ...
  python api_scorecard.py --output scorecard.txt openapi.json
    ...
    parser.add_argument("spec_file", help="OpenAPI/Swagger specification file (JSON format)")
    ...
    if args.min_grade:
        ...
        if current_grade_index < min_grade_index:
            print(
                f"Grade {scorecard.overall_grade} is below minimum required grade {args.min_grade}",
                file=sys.stderr,
            )
            return 1
