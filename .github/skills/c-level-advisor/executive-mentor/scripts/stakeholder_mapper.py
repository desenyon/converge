...
def calculate_overall_alignment(stakeholders: list[dict]) -> dict:
    ...
    weighted_alignment = (
        sum(s["alignment"] * s["influence"] for s in stakeholders) / total_influence
    )
...
def render_grid(stakeholders: list[dict], width: int = 60) -> str:
    ...
    lines.append("         Low alignment              High alignment")
    lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--sample", action="store_true", help="Print sample JSON structure and exit"
    )
