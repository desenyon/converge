...
def compute_hire_costs(hire: HireTarget) -> dict:
    ...
    total_comp = (
        ...
        + hire.equity_annual_usd
        + hire.benefits_annual
    )
...
def compute_totals(plan: HiringPlan) -> dict:
    ...
    revenue_per_employee_current = (
        plan.current_revenue / plan.current_headcount if plan.current_headcount > 0 else 0
    )
    ...
    return {
        ...
        "headcount_growth_pct": (
            (final_headcount - plan.current_headcount) / plan.current_headcount * 100
        )
        if plan.current_headcount > 0
        else 0,
        "total_annual_comp_added": total_comp,
        "total_first_year_cost": total_first_year,
        ...
        risks.append(
            {
                ...
                "recommendation": "Internal recruiter at $120–150K fully loaded pays off at 3–4 hires/year vs. agency fees.",
            }
        )
...
def print_report(plan: HiringPlan):
    ...
    print(sep)
    print(
        f"  {'Quarter':<10} {'New Hires':>10} {'HC (EOP)':>10} {'Comp Added':>14} {'1yr Cost':>14} {'Recruiter $':>12}"
    )
    print(f"  {'-' * 10} {'-' * 10} {'-' * 10} {'-' * 14} {'-' * 14} {'-' * 12}")
    for q, data in q_summary.items():
        print(
            f"  {q:<10} {data['new_hires']:>10} {data['headcount_eop']:>10} "
    ...
    for h in sorted(plan.hires, key=lambda x: quarter_to_sortkey(x.quarter)):
        ...
        print(
            f"  {h.role:<30} {h.function:<14} {h.level:<6} {h.quarter:<8} "
...
def build_sample_plan() -> HiringPlan:
    ...
    plan.hires = [
        ...
        HireTarget(
            ...
            base_salary=185_000,
            bonus_pct=0.0,
            equity_annual_usd=25_000,
            benefits_annual=18_000,
            recruiter_fee_pct=0.0,
            ramp_months=2,
            priority="High",
            open_to_internal=True,
        ...
        HireTarget(
            ...
            base_salary=95_000,
            bonus_pct=0.50,
            equity_annual_usd=10_000,
            benefits_annual=15_000,
            recruiter_fee_pct=0.18,
            ramp_months=4,
            priority="High",
        ...
        HireTarget(
            ...
            base_salary=175_000,
            bonus_pct=0.10,
            equity_annual_usd=22_000,
            benefits_annual=18_000,
            recruiter_fee_pct=0.20,
            ramp_months=3,
            priority="High",
...
Examples:
  ...
  python hiring_plan_modeler.py --export-json         # Output plan as JSON template
    ...
    parser.add_argument(
        "--export-json", action="store_true", help="Export sample plan as JSON template"
    )
