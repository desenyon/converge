...
class Employee:
    ...
    last_equity_refresh_months_ago: int | None = None
...
def find_band(roster: CompRoster, level: str, function: str, zone: str) -> BandDefinition | None:
    """Find best-matching band. Falls back to any matching level+function if zone not found."""
    matches = [
        b
        for b in roster.bands
        if b.level == level and b.function == function and b.location_zone == zone
    ]
    if matches:
        return matches[0]
...
def analyze_employee(emp: Employee, roster: CompRoster) -> dict:
    ...
    if band:
        ...
        if emp.equity_vest_years_remaining < 1.0 and (
            emp.last_equity_refresh_months_ago is None or emp.last_equity_refresh_months_ago > 24
        ):
            result["flags"].append(
...
def pay_equity_audit(analyses: list[dict], employees: list[Employee]) -> dict:
    ...
    def group_stats(group_key_fn):
        ...
        return {
            k: {
                "n": len(v),
                "avg_cr": round(sum(v) / len(v), 3),
                "min_cr": round(min(v), 3),
                "max_cr": round(max(v), 3),
            }
...
def print_report(roster: CompRoster):
    ...
    for fn in functions:
        ...
        for a in sorted(fn_analyses, key=lambda x: -x["base"]):
            ...
            flag_summary = ", ".join(
                s for s, _ in a["flags"] if s in ("CRITICAL", "HIGH", "MEDIUM")
            )
            flag_str = flag_summary if flag_summary else "OK"
            print(
                f"  {a['name']:<22} {a['role']:<28} {a['level']:<5} "
    ...
    if below_min or below_mid:
        ...
        if below_mid:
            cost_to_90 = sum(
                int(a["band"].band_mid * 0.90) - a["base"]
                for a in below_mid
                if a["base"] < int(a["band"].band_mid * 0.90)
            )
...
def build_sample_roster() -> CompRoster:
    ...
    roster.bands = [
        BandDefinition(
            "L2", "Engineering", 115_000, 132_000, 155_000, 110_000, 132_000, 155_000, "Tier1"
        ),
        BandDefinition(
            "L3", "Engineering", 148_000, 170_000, 198_000, 145_000, 170_000, 198_000, "Tier1"
        ),
        BandDefinition(
            "L4", "Engineering", 185_000, 215_000, 248_000, 182_000, 215_000, 250_000, "Tier1"
        ),
        BandDefinition(
            "M1", "Engineering", 170_000, 195_000, 225_000, 168_000, 195_000, 225_000, "Tier1"
        ),
        BandDefinition(
            "L2", "Engineering", 95_000, 108_000, 125_000, 92_000, 108_000, 126_000, "Tier2"
        ),
        BandDefinition(
            "L3", "Engineering", 122_000, 140_000, 162_000, 120_000, 140_000, 162_000, "Tier2"
        ),
        ...
        BandDefinition(
            "M1", "Sales", 130_000, 150_000, 172_000, 128_000, 150_000, 172_000, "Tier1"
        ),
        BandDefinition(
            "L2", "Product", 125_000, 145_000, 168_000, 123_000, 145_000, 168_000, "Tier1"
        ),
        BandDefinition(
            "L3", "Product", 155_000, 178_000, 205_000, 153_000, 178_000, 205_000, "Tier1"
        ),
    ...
    roster.employees = [
        ...
        Employee(
            ...
            equity_shares=40_000,
            equity_strike=1.50,
            equity_current_409a=6.80,
            equity_vest_years_remaining=2.5,
            benefits_annual=18_000,
            gender="M",
            ethnicity="Asian",
            tenure_years=2.5,
            performance_rating=4,
            last_raise_months_ago=14,
        ...
        Employee(
            ...
            base_salary=152_000,
            bonus_target_pct=0.0,
            equity_shares=30_000,
            equity_strike=2.20,
            equity_current_409a=6.80,
            equity_vest_years_remaining=0.5,
            benefits_annual=18_000,
            gender="F",
            ethnicity="Asian",
            tenure_years=3.8,
            performance_rating=5,
            last_raise_months_ago=11,
        ...
        Employee(
            ...
            base_salary=110_000,
            bonus_target_pct=0.0,
            equity_shares=15_000,
            equity_strike=2.50,
            equity_current_409a=6.80,
            equity_vest_years_remaining=3.0,
            benefits_annual=15_000,
            gender="M",
            ethnicity="Black",
            tenure_years=1.2,
            performance_rating=3,
            last_raise_months_ago=12,
            last_equity_refresh_months_ago=None,
        ),
        ...
        Employee(
            ...
            base_salary=222_000,
            bonus_target_pct=0.0,
            equity_shares=60_000,
            equity_strike=0.80,
            equity_current_409a=6.80,
            equity_vest_years_remaining=2.0,
            benefits_annual=18_000,
            gender="F",
            ethnicity="Asian",
            tenure_years=4.2,
            performance_rating=5,
            last_raise_months_ago=8,
        ...
        Employee(
            ...
            base_salary=88_000,
            bonus_target_pct=0.0,
            equity_shares=12_000,
            equity_strike=3.00,
            equity_current_409a=6.80,
            equity_vest_years_remaining=2.5,
            benefits_annual=14_000,
            gender="M",
            ethnicity="Hispanic",
            tenure_years=1.8,
            performance_rating=4,
            last_raise_months_ago=22,
            last_equity_refresh_months_ago=None,
        ),
        ...
        Employee(
            ...
            "Tier1",
            base_salary=192_000,
            bonus_target_pct=0.10,
            equity_shares=35_000,
            equity_strike=1.20,
            equity_current_409a=6.80,
            equity_vest_years_remaining=1.8,
            benefits_annual=18_000,
            gender="F",
            ethnicity="Asian",
            tenure_years=2.8,
            performance_rating=4,
            last_raise_months_ago=9,
        ...
        Employee(
            ...
            "Tier1",
            base_salary=155_000,
            bonus_target_pct=0.20,
            equity_shares=20_000,
            equity_strike=2.00,
            equity_current_409a=6.80,
            equity_vest_years_remaining=1.5,
            benefits_annual=16_000,
            gender="NB",
            ethnicity="White",
            tenure_years=2.2,
            performance_rating=3,
            last_raise_months_ago=10,
        ...
        Employee(
            ...
            base_salary=176_000,
            bonus_target_pct=0.10,
            equity_shares=22_000,
            equity_strike=1.80,
            equity_current_409a=6.80,
            equity_vest_years_remaining=2.0,
            benefits_annual=17_000,
            gender="F",
            ethnicity="Asian",
            tenure_years=2.0,
            performance_rating=4,
        ...
        Employee(
            ...
            "Tier1",
            base_salary=82_000,
            bonus_target_pct=0.08,
            equity_shares=5_000,
            equity_strike=4.00,
            equity_current_409a=6.80,
            equity_vest_years_remaining=3.5,
            benefits_annual=14_000,
            gender="F",
...
Examples:
  ...
  python comp_benchmarker.py --export-json            # Output JSON template
    ...
    parser.add_argument(
        "--export-json", action="store_true", help="Export sample roster as JSON template"
    )
