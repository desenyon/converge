...
def calculate_framework_coverage(controls: list[dict]) -> dict:
    ...
    for fw in FRAMEWORKS:
        ...
        remaining_effort = sum(
            c["effort_days"] for c in applicable if c["status"] not in ("Implemented", "Verified")
        )
        total_cost = sum(c["cost_usd"] for c in applicable)
        remaining_cost = sum(
            c["cost_usd"] for c in applicable if c["status"] not in ("Implemented", "Verified")
        )
...
def find_high_leverage_controls(controls: list[dict]) -> list[dict]:
    ...
    multi_fw = [
        c
        for c in controls
        if c["framework_count"] >= 3 and c["status"] not in ("Implemented", "Verified")
    ]
...
def print_control_table(controls: list[dict], framework_filter: str | None = None):
    filtered = controls
    if framework_filter:
    ...
    print("-" * 90)
    header = (
        f"{'ID':<14} {'Control Name':<30} {'Frameworks':<8} {'Effort':<8} {'Cost':<10} {'Status'}"
    )
    print(header)
    print("-" * 90)
