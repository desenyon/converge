...
class ProjectMetrics:
    ...
    def __init__(self, data: dict[str, Any]):
        ...
        self.actual_start: str | None = data.get("actual_start")
        ...
        self.team_satisfaction: float | None = data.get("team_satisfaction")

        # Stakeholder metrics
        self.stakeholder_satisfaction: float | None = data.get("stakeholder_satisfaction")
...
def analyze_portfolio_dimensions(project_scores: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for dimension in HEALTH_DIMENSIONS.keys():
        scores = [project["dimension_scores"][dimension]["score"] for project in project_scores]
...
def generate_rag_status_summary(project_scores: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    rag_percentages = {
        status: (count / max(total_projects, 1)) * 100 for status, count in rag_counts.items()
    }
    ...
    return {
        ...
        "green_projects": [
            {"id": p["project_id"], "name": p["project_name"], "score": p["composite_score"]}
...
def identify_intervention_priorities(project_scores: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    intervention_projects = [
        p for p in project_scores if p["intervention_level"] in ["immediate", "urgent", "monitor"]
    ]
...
def _identify_risk_factors(project: dict[str, Any]) -> list[str]:
    ...
    dimension_scores = project["dimension_scores"]
    poor_dimensions = [dim for dim, data in dimension_scores.items() if data["score"] < 50]
...
ProjectMetrics.calculate_composite_health_score = lambda self: calculate_project_health_score(self)[
    "composite_score"
]
