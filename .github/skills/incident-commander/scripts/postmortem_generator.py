...
THEME_RECS = {
    "process": [
        "Establish a quarterly process review cadence covering change management and deployment procedures.",
    ...
    "tooling": [
        "Schedule quarterly reviews of alerting thresholds and monitoring coverage.",
    ...
    "human": [
        "Build structured onboarding that covers incident-prone areas and past postmortems.",
    ...
    "environment": [
        "Conduct periodic capacity planning reviews using production traffic replays.",
    ...
    "external": [
        "Perform formal SLA reviews for all third-party dependencies annually.",
...
class TimelineMetrics:
    ...
    def _delta_min(self, start: datetime | None, end: datetime | None) -> float | None:
        if start is None or end is None:
            return None
        return round((end - start).total_seconds() / 60.0, 1)

    @property
    def mttd(self) -> float | None:
    ...
    def benchmark_comparison(self) -> dict[str, dict[str, Any]]:
        ...
        if h is not None:
            results["postmortem_timeliness"] = {
                "actual_hours": h,
                "target_hours": POSTMORTEM_TARGET_HOURS,
                "met_target": self.postmortem_on_time,
                "delta_hours": round(h - POSTMORTEM_TARGET_HOURS, 1),
            }
        return results

    def to_dict(self) -> dict[str, Any]:
        return {
            "mttd_minutes": self.mttd,
...
class ContributingFactor:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "weight": self.weight,
            "mapped_action_type": self.mapped_action_type,
        }
...
class FiveWhysAnalysis:
    ...
    def __init__(self, factor: ContributingFactor) -> None:
        ...
        self.chain: list[str] = [f"Why? {factor.description}"] + WHY_TEMPLATES.get(
            factor.category, WHY_TEMPLATES["process"]
        )

    def to_dict(self) -> dict[str, Any]:
        return {
            "factor": self.factor.description,
            "category": self.factor.category,
            "chain": self.chain,
            "systemic_theme": self.systemic_theme,
        }
...
class ActionItem:
    ...
    def _score_quality(self) -> int:
        ...
        if self.deadline:
            s += 20
        if self.priority in PRIORITY_ORDER:
            s += 10
        if self.type in ACTION_TYPES:
            s += 10
        if any(
            kw in self.title.lower()
            for kw in ["%", "threshold", "within", "before", "after", "less than", "greater than"]
        ):
            s += 10
    ...
    def is_past_deadline(self) -> bool:
        if not self.deadline or self.status != "open":
            return False
        try:
            dl = datetime.strptime(self.deadline, "%Y-%m-%d").replace(tzinfo=UTC)
            return datetime.now(UTC) > dl
        except ValueError:
            return False
...
class PostmortemReport:
    ...
    def _compute_factor_distribution(self) -> dict[str, float]:
        dist: dict[str, float] = dict.fromkeys(FACTOR_CATEGORIES, 0.0)
        ...
        for cat in factor_cats:
            ...
            if expected and expected not in action_types:
                suggestions.append(
                    {
                        "type": expected,
                        "suggestion": MISSING_ACTION_TEMPLATES.get(
                            expected, "Add an action item for this gap"
                        ),
    ...
    def customer_impact_summary(self) -> dict[str, Any]:
        ...
        sev = (
            "high"
            if (affected > 10000 or revenue > 50000)
            else ("medium" if (affected > 1000 or revenue > 5000) else "low")
        )
        return {
            "affected_users": affected,
            "failed_transactions": failed_tx,
            "revenue_impact_usd": revenue,
            "data_loss": data_loss,
            "data_integrity": "compromised" if data_loss else "intact",
            "customer_communication_required": comm_required,
            "impact_severity": sev,
        }
    ...
    def executive_summary(self) -> str:
        ...
        parts = [
            ...
            "Data loss was confirmed; affected customers must be notified."
            if ci["data_loss"]
    ...
    def overdue_p1_items(self) -> list[dict[str, str]]:
        return [
            ...
            if a.priority in ("P0", "P1") and a.is_past_deadline
        ]
...
def _generate_lessons(report: PostmortemReport) -> list[str]:
    ...
    avg_q = (
        sum(a.quality_score for a in report.action_items) / len(report.action_items)
    ...
    if not lessons:
        lessons.append(
            "This incident was handled within benchmarks. Continue reinforcing "
...
def format_text(report: PostmortemReport) -> str:
    ...
    for label, val, unit in [
        ("MTTD (Time to Detect)", tm.mttd, "min"),
    ...
    for name, d in tm.benchmark_comparison().items():
        if "actual_minutes" in d:
            ...
            L.append(
                f"  {name:<25s} actual={d['actual_minutes']}min  benchmark={d['benchmark_minutes']}min  [{st}]"
            )
        elif "actual_hours" in d:
            st = "PASS" if d["met_target"] else "FAIL"
            L.append(
                f"  {name:<25s} actual={d['actual_hours']}hrs  target={d['target_hours']}hrs  [{st}]"
            )
    # Customer Impact
    h1("CUSTOMER IMPACT")
    ...
    L.append(
        f"  Comms required:          {'Yes' if ci['customer_communication_required'] else 'No'}"
    )
    # Root Cause
    ...
    L.append("")
    hdr = (
        f"  {'Priority':<10s} {'Type':<14s} {'Owner':<25s} {'Deadline':<12s} {'Quality':<8s} Title"
    )
    L.append(hdr)
    L.append("  " + "-" * (len(hdr) - 2))
    for a in sorted(report.action_items, key=lambda x: PRIORITY_ORDER.get(x.priority, 99)):
        ...
        L.append(
            f"  {a.priority:<10s} {a.type:<14s} {a.owner:<25s} {a.deadline:<12s} "
    ...
    if overdue:
        ...
        for item in overdue:
            L.append(
                f"  OVERDUE: {item['title']} (owner: {item['owner']}, deadline: {item['deadline']})"
            )
    # Participants
    h1("PARTICIPANTS")
    L.append("")
...
def format_markdown(report: PostmortemReport) -> str:
    ...
    L.append("|--------|-------|-----------|--------|")
    labels = {
        "mttd": "MTTD (Time to Detect)",
        "mttr": "MTTR (Time to Resolve)",
        "time_to_mitigate": "Time to Mitigate",
        "time_to_declare": "Time to Declare",
    ...
    for key, label in labels.items():
        ...
        if b and "actual_minutes" in b:
            ...
            L.append(
                f"| {label} | {b['actual_minutes']} min | {b['benchmark_minutes']} min | {st} |"
            )
    ...
    L.append(f"- **Impact severity:** {ci['impact_severity']}")
    L.append(
        f"- **Customer communication required:** {'Yes' if ci['customer_communication_required'] else 'No'}"
    )
    L.append("")
    # Root Cause Analysis
    ...
    for a in sorted(report.action_items, key=lambda x: PRIORITY_ORDER.get(x.priority, 99)):
        flag = " **OVERDUE**" if a.is_past_deadline else ""
        L.append(
            f"| {a.priority} | {a.type} | {a.owner} | {a.deadline} | {a.quality_score}/100 | {a.title}{flag} |"
        )
    L.append("")
    if report.coverage_gaps:
    ...
    if overdue:
        ...
        for item in overdue:
            L.append(
                f"- **{item['title']}** (owner: {item['owner']}, deadline: {item['deadline']})"
            )
        L.append("")
    # Participants
...
def load_input(filepath: str | None) -> dict[str, Any]:
    ...
    if filepath:
        ...
        if sys.stdin.isatty():
            ...
            print(
                "Usage: postmortem_generator.py [data_file] or pipe JSON via stdin.",
                file=sys.stderr,
            )
            sys.exit(1)
        try:
