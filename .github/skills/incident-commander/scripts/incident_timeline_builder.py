...
class IncidentEvent:
    ...
    def __init__(self, data: dict[str, Any]):
        ...
        self.timestamp: datetime | None = _parse_timestamp(self.timestamp_raw)
...
class TimelineGap:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "start": self.start.strftime(ISO_FORMAT),
...
class TimelineAnalysis:
    ...
    def __init__(self):
        ...
        self.declared_at: datetime | None = None
...
def parse_incident_data(data: dict[str, Any]) -> TimelineAnalysis:
    ...
    for raw in raw_events:
        ...
        if event.timestamp is None:
            a.errors.append(
                f"Skipping event with unparseable timestamp: {raw.get('timestamp', '')}"
            )
            continue
...
def detect_phases(analysis: TimelineAnalysis) -> None:
    ...
    current: IncidentPhase | None = None
...
def generate_communications(analysis: TimelineAnalysis) -> None:
    ...
    if resolved:
        ...
        sp_body = (
            f"The incident affecting {svc} has been resolved.\n\n"
        ...
        sp_body = (
            f"We are investigating degraded performance in {svc}. "
            f"Affected services: {affected}.\n\n"
            f"Our team is working to identify the root cause. Updates every 30 minutes."
        )
    analysis.communications.append(
        CommunicationTemplate("status_page", "external", sp_subj, sp_body)
    )

    # 3 -- Executive summary
    phase_lines = (
        "\n".join(f"  - {p.name}: {_fmt_duration(p.duration_minutes)}" for p in analysis.phases)
    ...
    if resolved:
        cust_body = (
            f"We experienced an issue affecting {svc} starting at {decl}.\n\n"
        ...
        cust_body = (
            f"We are experiencing an issue affecting {svc} starting at {decl}.\n\n"
...
def format_text_output(analysis: TimelineAnalysis) -> str:
    ...
    L.append("-" * 32)
    L.append(
        f"  MTTD (Mean Time to Detect):   {_fmt_duration(analysis.metrics.get('mttd_minutes'))}"
    )
    L.append(
        f"  MTTR (Mean Time to Resolve):  {_fmt_duration(analysis.metrics.get('mttr_minutes'))}"
    )
    ...
    L.append(
        f"  Timeline Gaps (>{GAP_THRESHOLD_MINUTES}m):      {analysis.metrics.get('gap_count', 0)}"
    )
    L.append("")
    ...
    if analysis.phases:
        for p in analysis.phases:
            L.append(
                f"  [{_fmt_ts(p.start_time)} - {_fmt_ts(p.end_time)}]  {p.name} ({_fmt_duration(p.duration_minutes)})"
            )
    ...
    if analysis.gaps:
        ...
        for g in analysis.gaps:
            L.append(
                f"  WARNING: {_fmt_duration(g.duration_minutes)} gap between {_fmt_ts(g.start)} and {_fmt_ts(g.end)}"
            )
        L.append("")
...
def format_json_output(analysis: TimelineAnalysis) -> dict[str, Any]:
    ...
    return {
        "incident": {
            ...
            "declared_at": analysis.declared_at.strftime(ISO_FORMAT)
            if analysis.declared_at
            else None,
            "resolved_at": analysis.resolved_at.strftime(ISO_FORMAT)
...
def format_markdown_output(analysis: TimelineAnalysis) -> str:
    ...
    if analysis.phases:
        ...
        for p in analysis.phases:
            L.append(
                f"| {p.name} | {_fmt_ts(p.start_time)} | {_fmt_ts(p.end_time)} | {_fmt_duration(p.duration_minutes)} | {len(p.events)} |"
            )
        L.append("")
        # ASCII bar chart
        max_dur = max(
            (p.duration_minutes for p in analysis.phases if p.duration_minutes), default=0
        )
    ...
    if analysis.gaps:
        ...
        L.append("")
        L.append(
            f"> {len(analysis.gaps)} gap(s) of >{GAP_THRESHOLD_MINUTES} minutes detected. "
            f"These may represent blind spots where important activity was not recorded."
        )
        L.append("")
        for g in analysis.gaps:
            L.append(
                f"- **{_fmt_duration(g.duration_minutes)}** gap from `{_fmt_ts(g.start)}` to `{_fmt_ts(g.end)}`"
            )
        L.append("")
...
def main() -> int:
    ...
    try:
        if args.data_file:
            ...
            if sys.stdin.isatty():
                print(
                    "Error: No input file specified and stdin is a terminal. "
                    "Provide a file argument or pipe JSON to stdin.",
                    file=sys.stderr,
                )
                return 1
            try:
        ...
        if "incident" not in raw_data and "events" not in raw_data:
            print(
                "Error: Input must contain at least 'incident' or 'events' keys.", file=sys.stderr
            )
            return 1
