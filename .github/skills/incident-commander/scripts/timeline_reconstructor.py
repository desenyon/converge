...
Event = namedtuple(
    "Event", ["timestamp", "source", "type", "message", "severity", "actor", "metadata"]
)
...
class TimelineReconstructor:
        ...
        for event_dict in events_data:
            try:
                ...
                severity = self._parse_severity(
                    event_dict.get("severity", event_dict.get("level", "unknown"))
                )
                ...
                metadata = {
                    ...
                    if k
                    not in ["timestamp", "time", "source", "type", "message", "severity", "actor"]
                }
    ...
    def _detect_phases(self, events: list[Event]) -> list[Phase]:
            ...
            if detected_phase != current_phase:
                ...
                if current_phase and phase_events:
                    phase_obj = Phase(
                        ...
                        duration=(
                            phase_events[-1].timestamp - phase_events[0].timestamp
                        ).total_seconds()
                        / 60,
        ...
        if current_phase and phase_events:
            phase_obj = Phase(
                ...
                start_time=phase_events[0].timestamp,
                end_time=phase_events[-1].timestamp,
                duration=(phase_events[-1].timestamp - phase_events[0].timestamp).total_seconds()
                / 60,
                events=phase_events,
                description=self.phase_patterns[current_phase]["description"],
            )
    ...
    def _merge_adjacent_phases(self, phases: list[Phase]) -> list[Phase]:
        ...
        for next_phase in phases[1:]:
            if (
                next_phase.name == current_phase.name
                and (next_phase.start_time - current_phase.end_time).total_seconds() < 300
            ):  # 5 min gap
                # Merge phases
        ...
        return {
            ...
            "activity_metrics": {
                ...
                "events_per_hour": round(
                    (len(events) / (total_duration / 60)) if total_duration > 0 else 0, 1
                ),
    ...
    def _analyze_gaps(self, events: list[Event], phases: list[Phase]) -> dict[str, Any]:
        ...
        for i in range(len(phases) - 1):
            ...
            threshold = self.gap_thresholds.get(
                threshold_key, self.gap_thresholds["phase_transition"]
            )
        ...
        for i in range(len(comm_events) - 1):
            gap_minutes = (
                comm_events[i + 1].timestamp - comm_events[i].timestamp
            ).total_seconds() / 60
                ...
                gaps.append(
                    {
                        ...
                        "severity": "warning"
                        if gap_minutes < self.gap_thresholds["communication_gap"] * 2
                        else "critical",
                    }
                )
...
Key milestones:"""
    ...
    def _create_phase_narrative(self, phase: Phase, key_events: list[Event]) -> str:
        ...
        if phase.name == "detection":
            ...
            actions = [
                e.message
                for e in key_events
                if "investigating" in e.message.lower() or "checking" in e.message.lower()
            ]
            investigation_text = (
                "performed various diagnostic activities"
                if not actions
                else f"focused on {actions[0]}"
            )
            return template.format(
                first_event=first_event, investigation_actions=investigation_text
            )
        else:
    ...
    def _generate_summary(
        self, events: list[Event], phases: list[Phase], metrics: dict
    ) -> dict[str, Any]:
...
def format_text_output(result: dict) -> str:
    ...
    if "gap_analysis" in result and result["gap_analysis"]["gaps"]:
        ...
        for gap in result["gap_analysis"]["gaps"][:5]:  # Show first 5 gaps
            output.append(
                f"  {gap['type'].replace('_', ' ').title()}: {gap['gap_minutes']} min gap (threshold: {gap['threshold_minutes']} min)"
            )
        output.append("")
...
def format_markdown_output(result: dict) -> str:
    ...
    for event in timeline["events"]:
        ...
        output.append(
            f"**{timestamp.strftime('%H:%M:%S')}** [{event['source']}] {event['message']}"
        )
        output.append("")
...
Input JSON format:
  [
    ...
    parser.add_argument("--input", "-i", help="Input file path (JSON format) or '-' for stdin")
    ...
    parser.add_argument(
        "--detect-phases", action="store_true", help="Enable advanced phase detection"
    )

    parser.add_argument(
        "--gap-analysis", action="store_true", help="Perform gap analysis on timeline"
    )

    parser.add_argument(
        "--min-events", type=int, default=1, help="Minimum number of events required (default: 1)"
    )
