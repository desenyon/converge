...
""",
    ...
    def _extract_incident_info(self, incident_data: dict) -> dict[str, Any]:
        ...
        return {
            ...
            "start_time": self._parse_timestamp(
                incident_data.get("start_time", incident_data.get("timestamp", ""))
            ),
    ...
    def _perform_rca(
        self, incident_data: dict, timeline_data: dict | None, method: str
    ) -> dict[str, Any]:
    ...
    def _five_whys_analysis(
        self, incident_data: dict, timeline_data: dict | None
    ) -> dict[str, Any]:
            ...
            whys.append(
                {
                    ...
                    "evidence": self._find_supporting_evidence(
                        answer, incident_data, timeline_data
                    ),
                }
            )
    ...
    def _fishbone_analysis(self, incident_data: dict, timeline_data: dict | None) -> dict[str, Any]:
    ...
    def _timeline_analysis(self, incident_data: dict, timeline_data: dict | None) -> dict[str, Any]:
    ...
    def _bow_tie_analysis(self, incident_data: dict, timeline_data: dict | None) -> dict[str, Any]:
    ...
    def _infer_why_answers(self, incident_data: dict, timeline_data: dict | None) -> list[str]:
    ...
    def _find_supporting_evidence(
        self, answer: str, incident_data: dict, timeline_data: dict | None
    ) -> list[str]:
        ...
        for key, value in metadata.items():
            if isinstance(value, str) and any(
                keyword in value.lower() for keyword in answer.lower().split()
            ):
    ...
    def _categorize_root_cause(self, cause: str) -> str:
        ...
        if any(
            ...
            for keyword in ["process", "procedure", "review", "change management"]
        ):
            return "Process"
        elif any(
            keyword in cause_lower for keyword in ["training", "knowledge", "skill", "experience"]
        ):
            return "People"
        elif any(
            keyword in cause_lower
            for keyword in ["system", "architecture", "code", "configuration"]
        ):
            return "Technology"
        elif any(
            keyword in cause_lower
            for keyword in ["network", "infrastructure", "dependency", "third-party"]
        ):
            return "Environment"
        else:
            return "Unknown"

    def _identify_category_factors(
        self, category: str, incident_data: dict, timeline_data: dict | None
    ) -> list[dict]:
        ...
        if category == "People":
            ...
            if "deployment" in description:
                factors.append(
                    {"factor": "Insufficient deployment validation", "likelihood": "high"}
                )
    ...
    def _identify_primary_factors(self, categories: dict) -> list[dict]:
        ...
        for category_name, category_data in categories.items():
            high_likelihood_factors = [
                f for f in category_data["factors"] if f.get("likelihood") == "high"
            ]
            primary_factors.extend(
                [{**factor, "category": category_name} for factor in high_likelihood_factors]
            )

        return primary_factors

    def _synthesize_fishbone_root_causes(
        self, categories: dict, primary_factors: list[dict]
    ) -> list[dict]:
        ...
        for category, factors in category_factors.items():
            if len(factors) > 1:
                ...
                root_causes.append(
                    {"cause": factors[0]["factor"], "category": category, "confidence": "medium"}
                )
    ...
    def _extract_timeline_root_causes(
        self, decision_points: list, missed_opportunities: list, response_analysis: dict
    ) -> list[dict]:
    ...
    def _identify_threats(self, incident_data: dict, timeline_data: dict | None) -> list[dict]:
    ...
    def _identify_consequences(self, incident_data: dict) -> list[dict]:
        ...
        if "all users" in customer_impact or "complete outage" in customer_impact:
            consequences.append(
                {"consequence": "Complete service unavailability", "severity": "critical"}
            )
    ...
    def _identify_existing_barriers(
        self, incident_data: dict, timeline_data: dict | None
    ) -> list[dict]:
    ...
    def _recommend_additional_barriers(
        self, threats: list[dict], consequences: list[dict]
    ) -> list[dict]:
    ...
    def _generate_lessons_learned(
        self, incident_data: dict, timeline_data: dict | None, rca_results: dict
    ) -> dict[str, list[str]]:
    ...
    def _generate_action_items(
        self, incident_data: dict, rca_results: dict, lessons_learned: dict
    ) -> list[dict]:
    ...
    def _create_timeline_section(self, timeline_data: dict | None, severity: str) -> str:
        ...
        if "metrics" in timeline_data:
            ...
            timeline_content.append("")
            timeline_content.append(
                f"- Total Duration: {duration_metrics.get('total_duration_minutes', 'N/A')} minutes"
            )
            timeline_content.append(
                f"- Time to Mitigation: {duration_metrics.get('time_to_mitigation_minutes', 'N/A')} minutes"
            )
            timeline_content.append(
                f"- Time to Resolution: {duration_metrics.get('time_to_resolution_minutes', 'N/A')} minutes"
            )
            timeline_content.append("")
    ...
    def _generate_document_sections(
        ...
        sections["incident_date"] = (
            incident_info["start_time"].strftime("%Y-%m-%d %H:%M:%S UTC")
            if incident_info["start_time"]
            else "Unknown"
        )
        ...
        sections["responders"] = (
            ", ".join(incident_info["responders"]) if incident_info["responders"] else "TBD"
        )
        ...
        sections["what_went_wrong"] = self._create_what_went_wrong_section(
            rca_results, lessons_learned
        )
    ...
    def _create_executive_summary(self, incident_info: dict, rca_results: dict) -> str:
        ...
        summary_parts.append(
            f"On {incident_info['start_time'].strftime('%B %d, %Y') if incident_info['start_time'] else 'an unknown date'}, we experienced a {incident_info['severity']} incident affecting {incident_info.get('affected_services', ['our services'])}."
        )

        # Duration and impact
        summary_parts.append(
            f"The incident lasted {incident_info['duration']} and had the following impact: {incident_info['customer_impact']}"
        )
        ...
        if root_causes:
            ...
            summary_parts.append(
                f"Root cause analysis identified the primary issue as: {primary_cause}"
            )

        # Resolution
        summary_parts.append(
            f"The incident has been {incident_info['status']} and we have identified specific actions to prevent recurrence."
        )
    ...
    def _create_rca_section(self, rca_results: dict) -> str:
        ...
        rca_content.append(
            f"### Analysis Method: {self.rca_frameworks.get(method, {}).get('name', method)}"
        )
        rca_content.append("")
            ...
            for category, data in rca_results["categories"].items():
                if data["factors"]:
                    ...
                    for factor in data["factors"]:
                        rca_content.append(
                            f"- {factor['factor']} (likelihood: {factor.get('likelihood', 'unknown')})"
                        )
                    rca_content.append("")
    ...
    def _create_prevention_section(self, rca_results: dict, action_items: list[dict]) -> str:
        ...
        prevention_items = [
            item for item in action_items if "prevent" in item.get("description", "").lower()
        ]
    ...
    def _generate_metadata(
        self, incident_info: dict, rca_results: dict, action_items: list[dict]
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "critical_action_items": len(
                [item for item in action_items if item.get("priority") == "P0"]
            ),
            ...
            "review_completeness": self._assess_review_completeness(
                incident_info, rca_results, action_items
            ),
        }
    ...
    def _assess_review_completeness(
        self, incident_info: dict, rca_results: dict, action_items: list[dict]
    ) -> float:
...
def format_text_output(result: dict) -> str:
    ...
    output.append(
        f"  Prevention Timeline: {metadata.get('estimated_prevention_timeline', 'Unknown')}"
    )
...
Incident JSON format:
  {
    ...
    parser.add_argument("--incident", "-i", help="Incident data file (JSON) or '-' for stdin")

    parser.add_argument("--timeline", "-t", help="Timeline reconstruction file (JSON)")
    ...
    parser.add_argument(
        "--action-items", action="store_true", help="Generate detailed action items"
    )
