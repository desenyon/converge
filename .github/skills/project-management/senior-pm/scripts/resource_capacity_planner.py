...
ROLE_TYPES = {
    ...
    "product_manager": {
        ...
        "skill_multipliers": {"planning": 1.0, "stakeholder_mgmt": 1.0, "analysis": 0.9},
    },
    "designer": {
        "hourly_rate": 90,
        "efficiency_factor": 1.0,
    ...
    "qa_engineer": {
        ...
        "skill_multipliers": {"manual_testing": 1.0, "automation": 1.1, "performance": 0.9},
    },
}
...
PROJECT_COMPLEXITY_FACTORS = {"simple": 1.0, "moderate": 1.2, "complex": 1.5, "very_complex": 2.0}
...
class Resource:
    ...
    def __init__(self, data: dict[str, Any]):
        ...
        self.availability_end: str | None = data.get("availability_end")
...
def analyze_resource_utilization(resources: list[Resource]) -> dict[str, Any]:
    ...
    utilization_stats["overall_utilization"] = utilization_stats["total_allocated"] / max(
        utilization_stats["total_capacity"], 1
    )
    ...
    return {
        ...
        "utilization_categories": {
            k: [
                {"id": r.id, "name": r.name, "role": r.role, "utilization": r.current_utilization}
                for r in v
            ]
...
def analyze_project_capacity_requirements(projects: list[Project]) -> dict[str, Any]:
    ...
    for priority in ["high", "medium", "low"]:
        ...
        priority_distribution[priority] = {
            ...
            "weekly_demand": sum(
                p.required_hours_per_week for p in priority_projects if p.status != "completed"
            ),
        }
...
def optimize_resource_allocation(
    ...
    under_utilized = [
        r for r in resources if r.current_utilization < UTILIZATION_THRESHOLDS["under_utilized"]
    ]
    ...
    for project in projects:
        if project.capacity_gap > 0 and project.status != "completed":
            ...
            suitable_resources.sort(
                key=lambda x: (x["skill_match_score"], x["available_hours"]), reverse=True
            )
...
def main() -> int:
    ...
    parser.add_argument("data_file", help="JSON file containing resource and project capacity data")
    parser.add_argument(
