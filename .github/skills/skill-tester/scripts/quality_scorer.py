...
class QualityReport:
    ...
    def _calculate_tier_recommendation(self):
        ...
        if self.overall_score >= 80 and all(
            score >= 75 for score in [doc_score, code_score, completeness_score, usability_score]
        ):
        ...
        # STANDARD tier requirements (most dimensions good)
            ...
            and sum(
                1
                for score in [doc_score, code_score, completeness_score, usability_score]
                if score >= 65
            )
            >= 3
        ):
    ...
    def _generate_improvement_roadmap(self):
        ...
        for dim_name, dimension in self.dimensions.items():
            for suggestion in dimension.suggestions:
                priority = (
                    "HIGH" if dimension.score < 60 else "MEDIUM" if dimension.score < 75 else "LOW"
                )
    ...
    def _calculate_summary_stats(self):
        ...
        self.summary_stats = {
            "highest_dimension": max(self.dimensions.items(), key=lambda x: x[1].score)[0]
            if scores
            else "None",
            "lowest_dimension": min(self.dimensions.items(), key=lambda x: x[1].score)[0]
            if scores
            else "None",
            "score_variance": sum((score - self.overall_score) ** 2 for score in scores)
            / len(scores)
            if scores
            else 0,
...
class QualityScorer:
    ...
    def assess_quality(self) -> QualityReport:
        ...
        try:
            ...
            self.log_verbose(
                f"Quality assessment completed. Overall score: {self.report.overall_score:.1f}"
            )
    ...
    def _score_documentation(self):
        ...
        dimension = QualityDimension(
            "Documentation", 0.25, "Quality of documentation and written materials"
        )
    ...
    def _score_skill_md(self, dimension: QualityDimension):
        ...
        try:
            ...
            dimension.add_score(
                "skill_md_length", length_score, 25, f"SKILL.md has {line_count} lines"
            )
            ...
            dimension.add_score(
                "skill_md_sections", section_score, 25, "Required and recommended section coverage"
            )

            # Score content depth
            depth_score = self._score_content_depth(content)
            dimension.add_score(
                "skill_md_depth", depth_score, 25, "Content depth and technical detail"
            )
    ...
    def _score_content_depth(self, content: str) -> float:
        ...
        depth_score = sum(
            1 for indicator in depth_indicators if indicator.lower() in content.lower()
        )
        ...
        example_count = sum(
            len(re.findall(pattern, content, re.IGNORECASE)) for pattern in example_patterns
        )
    ...
    def _score_examples(self, dimension: QualityDimension):
        ...
        dimension.add_score(
            "examples_availability", score, 25, f"Found {len(example_files)} example/sample files"
        )
    ...
    def _score_code_quality(self):
        ...
        dimension = QualityDimension(
            "Code Quality", 0.25, "Quality of Python scripts and implementation"
        )
    ...
    def _score_code_structure(self, python_files: list[Path], dimension: QualityDimension):
        ...
        if avg_structure_score < 15:
            dimension.add_suggestion(
                "Improve code structure with more functions, classes, and documentation"
            )
    ...
    def _score_output_support(self, python_files: list[Path], dimension: QualityDimension):
        ...
        for script_path in python_files:
            try:
                ...
                if any(
                    indicator in content for indicator in ['print(f"', 'print("', ".format(", 'f"']
                ):
    ...
    def _score_completeness(self):
        ...
        dimension = QualityDimension(
            "Completeness", 0.25, "Completeness of required components and assets"
        )
    ...
    def _score_directory_structure(self, dimension: QualityDimension):
        ...
        if missing_recommended:
            dimension.add_suggestion(
                f"Add recommended directories: {', '.join(missing_recommended)}"
            )
    ...
    def _score_expected_outputs(self, dimension: QualityDimension):
        ...
        dimension.add_score(
            "expected_outputs", score, 25, f"Expected outputs: {len(output_files)} files"
        )
    ...
    def _score_test_coverage(self, dimension: QualityDimension):
        ...
        dimension.add_score(
            "test_coverage", score, 25, f"Test coverage indicators: {len(test_files)} files"
        )
    ...
    def _score_installation(self, dimension: QualityDimension):
        ...
        if (self.skill_path / "requirements.txt").exists():
            ...
            dimension.add_suggestion(
                "Consider removing external dependencies for easier installation"
            )
        ...
        dimension.add_score(
            "installation_simplicity", max(score, 15), 25, "Installation complexity assessment"
        )
    ...
    def _score_help_accessibility(self, dimension: QualityDimension):
        ...
        dimension.add_score(
            "help_accessibility", min(score, 25), 25, "Help and documentation accessibility"
        )
    ...
    def _score_practical_examples(self, dimension: QualityDimension):
        ...
        dimension.add_score(
            "practical_examples", score, 25, f"Practical examples: {len(example_files)} files"
        )
...
class QualityReportFormatter:
    ...
    def format_human_readable(report: QualityReport, detailed: bool = False) -> str:
        ...
        for name, dimension in report.dimensions.items():
            lines.append(
                f"  {name}: {dimension.score:.1f}/100 ({dimension.weight * 100:.0f}% weight)"
            )
            if detailed and dimension.details:
                for component, details in dimension.details.items():
                    lines.append(
                        f"    • {component}: {details['score']:.1f}/{details['max_score']} - {details['details']}"
                    )
            lines.append("")
...
Letter Grades: A+ (95+), A (90+), A- (85+), B+ (80+), B (75+), B- (70+), C+ (65+), C (60+), C- (55+), D (50+), F (<50)
    ...
    parser.add_argument("--detailed", action="store_true", help="Show detailed component scores")
    parser.add_argument(
        "--minimum-score",
        type=float,
        default=0,
    ...
    try:
        ...
        if report.overall_score < args.minimum_score:
            print(
                f"\nERROR: Quality score {report.overall_score:.1f} is below minimum {args.minimum_score}",
                file=sys.stderr,
            )
            sys.exit(1)
