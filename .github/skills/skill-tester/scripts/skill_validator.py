...
class ValidationReport:
    ...
    def __init__(self, skill_path: str):
        ...
        self.timestamp = dt.datetime.now(dt.UTC).isoformat().replace("+00:00", "Z")
        self.checks = {}
        self.warnings = []
        self.errors = []
        self.suggestions = []
    ...
    def add_check(self, check_name: str, passed: bool, message: str = "", score: float = 0.0):
        ...
        self.checks[check_name] = {"passed": passed, "message": message, "score": score}
...
class SkillValidator:
    ...
    REQUIRED_SKILL_MD_SECTIONS = ["Name", "Description", "Features", "Usage", "Examples"]

    FRONTMATTER_REQUIRED_FIELDS = ["Name", "Tier", "Category", "Dependencies", "Author", "Version"]

    def __init__(self, skill_path: str, target_tier: str | None = None, verbose: bool = False):
        ...
        try:
            ...
            if line_count >= min_lines:
                self.report.add_check(
                    "skill_md_length", True, f"SKILL.md has {line_count} lines (≥{min_lines})", 1.0
                )
            else:
                self.report.add_check(
                    "skill_md_length", False, f"SKILL.md has {line_count} lines (<{min_lines})", 0.0
                )
                self.report.add_error(
                    f"SKILL.md too short: {line_count} lines, minimum {min_lines}"
                )
            ...
            self.report.add_check(
                "skill_md_readable", False, f"Error reading SKILL.md: {str(e)}", 0.0
            )
    ...
    def _validate_frontmatter(self, content: str):
        ...
        if content.startswith("---"):
            try:
                ...
                if end_marker == -1:
                    self.report.add_check(
                        "frontmatter_format", False, "Frontmatter closing marker not found", 0.0
                    )
                    return
                ...
                if not isinstance(frontmatter, dict):
                    self.report.add_check(
                        "frontmatter_format", False, "Frontmatter is not a valid dictionary", 0.0
                    )
                    return
                ...
                if not missing_fields:
                    self.report.add_check(
                        "frontmatter_complete", True, "All required frontmatter fields present", 1.0
                    )
                else:
                    self.report.add_check(
                ...
                self.report.add_check(
                    "frontmatter_format", False, f"Invalid YAML frontmatter: {str(e)}", 0.0
                )
    ...
    def _validate_required_sections(self, content: str):
        ...
        if not missing_sections:
            self.report.add_check("required_sections", True, "All required sections present", 1.0)
        else:
            self.report.add_check(
                "required_sections", False, f"Missing sections: {', '.join(missing_sections)}", 0.0
            )
    ...
    def _validate_readme(self):
        ...
        try:
            ...
            if len(content.strip()) >= 200:
                self.report.add_check(
                    "readme_substantial", True, "README.md has substantial content", 1.0
                )
            else:
                self.report.add_check(
                    "readme_substantial", False, "README.md content is too brief", 0.5
                )
                self.report.add_suggestion("Expand README.md with more detailed usage instructions")

        except Exception as e:
            self.report.add_check(
                "readme_readable", False, f"Error reading README.md: {str(e)}", 0.0
            )
    ...
    def _validate_directory_structure(self):
        ...
        for dir_name in required_dirs:
            ...
            if dir_path.exists() and dir_path.is_dir():
                self.report.add_check(
                    f"dir_{dir_name}_exists", True, f"{dir_name}/ directory found", 1.0
                )
            else:
                missing_required.append(dir_name)
                self.report.add_check(
                    f"dir_{dir_name}_exists", False, f"{dir_name}/ directory missing", 0.0
                )
        ...
        if missing_optional:
            self.report.add_suggestion(
                f"Consider adding optional directories: {', '.join(missing_optional)}"
            )
    ...
    def _validate_python_scripts(self):
        ...
        if len(python_files) >= min_scripts:
            self.report.add_check(
                "min_scripts_count",
                True,
                f"Found {len(python_files)} Python scripts (≥{min_scripts})",
                1.0,
            )
        else:
            self.report.add_check(
            ...
            self.report.add_error(
                f"Insufficient scripts: {len(python_files)}, minimum {min_scripts}"
            )
    ...
    def _validate_single_script(self, script_path: Path):
        ...
        try:
            ...
            if min_size <= loc <= max_size:
                self.report.add_check(
                    f"script_size_{script_name}",
                    True,
                    f"{script_name} has {loc} LOC (within {min_size}-{max_size})",
                    1.0,
                )
            else:
                self.report.add_check(
                    f"script_size_{script_name}",
                    False,
                    f"{script_name} has {loc} LOC (outside {min_size}-{max_size})",
                    0.5,
                )
                if loc < min_size:
                    self.report.add_suggestion(
                        f"Consider expanding {script_name} (currently {loc} LOC)"
                    )
                else:
                    self.report.add_suggestion(
                        f"Consider refactoring {script_name} (currently {loc} LOC)"
                    )
            ...
            try:
                ...
                self.report.add_check(
                    ...
                    True,
                    f"{script_name} has valid Python syntax",
                    1.0,
                )
                ...
                self.report.add_check(
                    f"script_syntax_{script_name}",
                    False,
                    f"{script_name} has syntax error: {str(e)}",
                    0.0,
                )
            ...
            self.report.add_check(
                f"script_readable_{script_name}", False, f"Cannot read {script_name}: {str(e)}", 0.0
            )
    ...
    def _validate_script_features(self, tree: ast.AST, script_name: str, content: str):
        ...
        required_features = self._get_tier_requirement(
            "features_required", ["argparse", "main_guard"]
        )
        ...
        if "argparse" in required_features:
            ...
            self.report.add_check(
                f"script_argparse_{script_name}",
                has_argparse,
                f"{'Uses' if has_argparse else 'Missing'} argparse in {script_name}",
                1.0 if has_argparse else 0.0,
            )
            if not has_argparse:
        ...
        if "main_guard" in required_features:
            ...
            self.report.add_check(
                f"script_main_guard_{script_name}",
                has_main_guard,
                f"{'Has' if has_main_guard else 'Missing'} main guard in {script_name}",
                1.0 if has_main_guard else 0.0,
            )
            if not has_main_guard:
                self.report.add_error(
                    f"{script_name} must have 'if __name__ == \"__main__\"' guard"
                )
        ...
        if not external_imports:
            self.report.add_check(
                f"script_imports_{script_name}",
                True,
                f"{script_name} uses only standard library",
                1.0,
            )
        else:
            self.report.add_check(
                f"script_imports_{script_name}",
                False,
                f"{script_name} uses external imports: {', '.join(external_imports)}",
                0.0,
            )
            self.report.add_error(
                f"{script_name} uses external imports: {', '.join(external_imports)}"
    ...
    def _validate_tier_compliance(self):
        ...
        if not failed_critical:
            self.report.add_check(
                "tier_compliance", True, f"Meets {self.target_tier} tier requirements", 1.0
            )
        else:
            self.report.add_check(
                "tier_compliance", False, f"Does not meet {self.target_tier} tier requirements", 0.0
            )
            self.report.add_error(
                f"Failed critical checks for {self.target_tier} tier: {', '.join(failed_critical)}"
            )
...
class ReportFormatter:
    ...
    def format_human_readable(report: ValidationReport) -> str:
        ...
        structure_checks = {
            k: v for k, v in report.checks.items() if k.startswith(("skill_md", "readme", "dir_"))
        }
        script_checks = {k: v for k, v in report.checks.items() if k.startswith("script_")}
        other_checks = {
            k: v
            for k, v in report.checks.items()
            if k not in structure_checks and k not in script_checks
        }
