...
class VisionModelTrainer:
    ...
    def generate_mmdetection_config(
        ...
        config = {
            ...
            "log_config": {
                "interval": 50,
                "hooks": [{"type": "TextLoggerHook"}, {"type": "TensorboardLoggerHook"}],
            },
    ...
    def generate_training_command(self) -> str:
        ...
        if self.framework == "ultralytics":
            ...
            return "python train_net.py --config-file config.yaml --num-gpus 1"
        elif self.framework == "mmdetection":
            return "python tools/train.py config.py"
        return ""
...
def main():
    ...
    parser.add_argument(
        "--task", choices=["detection", "segmentation"], default="detection", help="Task type"
    )
    parser.add_argument(
        "--framework",
    ...
    parser.add_argument(
        "--analyze-only", action="store_true", help="Only analyze dataset, do not generate config"
    )
