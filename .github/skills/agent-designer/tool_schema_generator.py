...
class ToolSchemaGenerator:
    ...
    def _generate_validation_rules(
        self, param_spec: dict[str, Any], param_type: ParameterType
    ) -> dict[str, Any]:
        ...
        if "pattern" in param_spec:
            ...
            format_name = self._detect_format(
                param_spec.get("name", ""), param_spec.get("description", "")
            )
    ...
    def _generate_integer_validation(self, param_spec: dict[str, Any]) -> dict[str, Any]:
        ...
        if "minimum" in param_spec:
            ...
            desc = param_spec.get("description", "").lower()
            if any(
                word in name + desc for word in ["count", "quantity", "amount", "size", "limit"]
            ):
                rules["minimum"] = 0
    ...
    def _detect_format(self, name: str, description: str) -> str | None:
    ...
    def generate_openai_schema(
        self, description: ToolDescription, input_params: list[ParameterSpec]
    ) -> dict[str, Any]:
        ...
        for param in input_params:
            prop_def = {"type": param.type.value, "description": param.description}
    ...
    def generate_tool_schema(self, description: ToolDescription) -> ToolSchema:
        ...
        for param in input_params:
            if param.validation_rules:
                validation_rules.append({"parameter": param.name, "rules": param.validation_rules})
...
def main():
    ...
    parser.add_argument("--validate", action="store_true", help="Validate generated schemas")
    ...
    try:
        ...
        if args.format == "both":
            ...
            # OpenAI format
            openai_schemas = {"functions": [schema.openai_schema for schema in schemas]}
            ...
            # Anthropic format
            anthropic_schemas = {"tools": [schema.anthropic_schema for schema in schemas]}
            ...
            # Usage examples
            examples_data = {"examples": {schema.name: schema.examples for schema in schemas}}
        ...
        print(
            f"Total input parameters: {sum(schema.metadata['input_parameters'] for schema in schemas)}"
        )
