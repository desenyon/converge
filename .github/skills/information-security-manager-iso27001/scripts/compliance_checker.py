...
ISO27001_CONTROLS = {
    ...
    "physical": {
        ...
        "controls": [
            ...
            {
                "id": "A.7.5",
                "name": "Protecting against environmental threats",
                "priority": "medium",
            },
...
def get_control_status(control_id: str, controls_data: dict | None = None) -> str:
...
def check_compliance(
    standard: str, controls_data: dict | None = None, domains: list[str] | None = None
) -> dict[str, Any]:
                    ...
                    results["findings"].append(
                        {
                            ...
                            "remediation": REMEDIATION_GUIDANCE.get(
                                control["id"], "Implement control per ISO 27001 requirements"
                            ),
                        }
                    )
    ...
    results["summary"]["compliance_percentage"] = (
        round(((implemented + partial * 0.5) / total) * 100, 1) if total > 0 else 0
    )
...
def format_output(results: dict[str, Any], gap_analysis: bool, output_format: str) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "| Domain | Implemented | Partial | Not Impl | Score |",
            "|--------|-------------|---------|----------|-------|",
        ]
    )
    ...
    for domain_key, domain_data in results["domains"].items():
        ...
        score = (
            round(((domain_data["implemented"] + domain_data["partial"] * 0.5) / total) * 100)
            if total > 0
            else 0
        )
        lines.append(
...
def main():
    parser = argparse.ArgumentParser(description="ISO 27001/27002 Compliance Checker")
    parser.add_argument(
        "--standard",
        "-s",
        required=True,
    ...
    parser.add_argument(
        "--controls-file", "-c", help="CSV file with current control implementation status"
    )
    parser.add_argument(
        "--gap-analysis",
        "-g",
