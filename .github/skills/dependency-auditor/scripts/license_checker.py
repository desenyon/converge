...
class DependencyLicense:
    ...
    license_detected: LicenseInfo | None
...
class LicenseChecker:
    ...
    def _build_license_database(self) -> dict[str, LicenseInfo]:
        ...
        return {
            ...
            "LGPL-2.1": LicenseInfo(
                ...
                obligations=[
                    "Source disclosure (library modifications)",
    ...
    def analyze_project(
        self, project_path: str, dependency_inventory: str | None = None
    ) -> dict[str, Any]:
        ...
        analysis_results["conflicts"] = self._detect_license_conflicts(
            analysis_results["project_license"], analysis_results["dependencies"]
        )

        # Calculate compliance score
        analysis_results["compliance_score"] = self._calculate_compliance_score(
            analysis_results["dependencies"], analysis_results["conflicts"]
        )
    ...
    def _detect_license_from_text(self, text: str) -> str | None:
    ...
    def _analyze_dependency_license(
        self, dependency: dict[str, Any], project_path: Path
    ) -> DependencyLicense:
    ...
    def _resolve_license_info(self, license_string: str) -> LicenseInfo | None:
    ...
    def _scan_package_directory(self, package_path: Path) -> LicenseInfo | None:
    ...
    def _detect_license_conflicts(
        self, project_license: str | None, dependencies: list[DependencyLicense]
        ...
        for dep in dependencies:
            ...
            if project_license in self.compatibility_matrix:
                compatibility = self.compatibility_matrix[project_license].get(
                    dep_license_id, False
                )

                if not compatibility:
                    severity = self._determine_conflict_severity(
                        project_license_info, dep.license_detected
                    )
                    ...
                    conflicts.append(
                        LicenseConflict(
                            ...
                            resolution_options=self._generate_conflict_resolutions(
    ...
    def _determine_conflict_severity(
        self, project_license: LicenseInfo, dep_license: LicenseInfo
    ) -> RiskLevel:
        """Determine severity of a license conflict."""
        if dep_license.license_type == LicenseType.UNKNOWN:
            return RiskLevel.CRITICAL
        elif (
            project_license.license_type == LicenseType.PERMISSIVE
            and dep_license.license_type == LicenseType.COPYLEFT_STRONG
        ):
            return RiskLevel.CRITICAL
    ...
    def _calculate_compliance_score(
        self, dependencies: list[DependencyLicense], conflicts: list[LicenseConflict]
    ) -> float:
        ...
        unknown_count = sum(
            1 for dep in dependencies if dep.license_detected.license_type == LicenseType.UNKNOWN
        )
        ...
        return {
            ...
            "conflict_summary": {
                ...
                "critical_conflicts": len(
                    [c for c in conflicts if c.severity == RiskLevel.CRITICAL]
                ),
    ...
    def _assess_distribution_risks(self, dependencies: list[DependencyLicense]) -> list[str]:
        ...
        if gpl_deps:
            risks.append(
                f"GPL dependencies require source code disclosure: {[d.name for d in gpl_deps]}"
            )
        ...
        if proprietary_deps:
            risks.append(
                f"Proprietary dependencies may require commercial licenses: {[d.name for d in proprietary_deps]}"
            )

        unknown_deps = [
            dep for dep in dependencies if dep.license_detected.license_type == LicenseType.UNKNOWN
        ]
        if unknown_deps:
            risks.append(
                f"Unknown licenses pose legal uncertainty: {[d.name for d in unknown_deps]}"
            )
    ...
    def _assess_commercial_risks(self, dependencies: list[DependencyLicense]) -> list[str]:
        ...
        agpl_deps = [dep for dep in dependencies if dep.license_detected.spdx_id == "AGPL-3.0"]
        if agpl_deps:
            risks.append(
                f"AGPL dependencies trigger copyleft for network services: {[d.name for d in agpl_deps]}"
            )
    ...
    def _generate_compliance_recommendations(self, analysis_results: dict[str, Any]) -> list[str]:
        ...
        critical_conflicts = [
            c for c in analysis_results["conflicts"] if c.severity == RiskLevel.CRITICAL
        ]
        if critical_conflicts:
            recommendations.append(
                "CRITICAL: Address license conflicts immediately before any distribution"
            )
        ...
        if unknown_count > 0:
            recommendations.append(
                f"Investigate and clarify licenses for {unknown_count} dependencies with unknown licensing"
            )
        ...
        if gpl_deps and analysis_results.get("project_license") in [
            "MIT",
            "Apache-2.0",
            "BSD-3-Clause",
        ]:
            recommendations.append(
                "Consider removing GPL dependencies or changing project license for permissive project"
            )
    ...
    def generate_report(self, analysis_results: dict[str, Any], format: str = "text") -> str:
            ...
            serializable_results["dependencies"] = [
                {
                    ...
                    "license_detected": asdict(dep.license_detected)
                    if dep.license_detected
                    else None,
                    "confidence": dep.confidence,
                }
                for dep in analysis_results["dependencies"]
            ]
            serializable_results["conflicts"] = [
                asdict(conflict) for conflict in analysis_results["conflicts"]
            ]
        ...
        if high_risk_deps:
            ...
            for dep in high_risk_deps[:10]:  # Top 10
                ...
                report.append(
                    f"  {dep.name} v{dep.version}: {license_name} ({dep.license_detected.risk_level.value.upper()})"
                )
            report.append("")
...
Examples:
  ...
  python license_checker.py /app --inventory deps.json --policy strict
    ...
    parser.add_argument("--inventory", help="Path to dependency inventory JSON file")
    parser.add_argument(
    ...
    parser.add_argument(
        "--warn-conflicts", action="store_true", help="Show warnings for potential conflicts"
    )
