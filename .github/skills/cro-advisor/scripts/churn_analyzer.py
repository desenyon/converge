...
class RetentionAnalyzer:
    ...
    def arr_waterfall(self, period_start, period_end):
        ...
        new_customers = [c for c in self.customers if period_start < c.start_date <= period_end]
        new_arr = sum(c.arr for c in new_customers)

        # Churned: were active at start, churn_date within period
        churned = [
            c
            for c in opening_customers
        ...
        nrr = (
            (opening_arr + expansion - contraction - churned_arr) / opening_arr
            if opening_arr
            else 0
        )
    ...
    def logo_churn_rate(self, period_start, period_end):
        ...
        churned = [c for c in opening if c.churn_date and period_start < c.churn_date <= period_end]
    ...
    def revenue_churn_rate(self, period_start, period_end):
        ...
        churned_arr = sum(
            c.arr for c in opening if c.churn_date and period_start < c.churn_date <= period_end
        )
...
class CohortAnalyzer:
    ...
    def retention_at_month(self, cohort_customers, months_after):
        ...
        retained_arr = sum(c.arr for c in cohort_customers if c.is_active(check_date))
...
class ExpansionAnalyzer:
    ...
    def expansion_summary(self):
        ...
        return {
            ...
            "net_expansion_rate": (total_expansion - total_contraction) / total_arr
            if total_arr
            else 0,
        }
    ...
    def expansion_by_segment(self):
        active = [c for c in self.customers if not c.is_churned()]
        by_segment = defaultdict(
            lambda: {"arr": 0.0, "expansion": 0.0, "contraction": 0.0, "count": 0}
        )
        for c in active:
        ...
        for seg, data in by_segment.items():
            ...
            result[seg] = {
                ...
                "net_nrr_contribution": (arr + data["expansion"] - data["contraction"]) / arr
                if arr
                else 0,
            }
        return result
...
def print_full_report(customers, period_start, period_end):
    ...
    print()
    print("  NRR Benchmark: >120% world-class | 100-120% healthy | <100% fix immediately")

    # ── Expansion summary
    print_section("EXPANSION REVENUE")
    exp = expansion_analyzer.expansion_summary()
    print(
        f"  Expanding customers:  {exp['expanding_count']} / {exp['active_customers']} ({fmt_pct(exp['expanding_count'] / exp['active_customers']) if exp['active_customers'] else '—'})"
    )
    ...
    h = (
        f"  {'Segment':<{col_w[0]}} {'Custs':>{col_w[1]}} {'ARR':>{col_w[2]}} "
    ...
    for seg, data in sorted(seg_data.items(), key=lambda x: -x[1]["arr"]):
        print(
            f"  {seg:<{col_w[0]}} {data['customer_count']:>{col_w[1]}} "
    ...
    for cohort, data in cohort_report.items():
        ...
        print(
            f"  {cohort:<10} {data['customer_count']:>6} "
    ...
    if at_risk:
        ...
        for acct in at_risk[:10]:  # Top 10
            ...
            print(
                f"  {acct['name']:<22} {acct['segment']:<14} "
    ...
    if len(at_risk) > len(active) * 0.20:
        flags.append(
            f"⚠️  {len(at_risk)} of {len(active)} active accounts flagged at-risk ({fmt_pct(len(at_risk) / len(active) if active else 0)})"
        )
...
def main():
    ...
    if args.output == "json":
        ...
        output = {
            ...
            "cohort_report": {
                k: {**v, "retention_curve": {str(m): r for m, r in v["retention_curve"].items()}}
