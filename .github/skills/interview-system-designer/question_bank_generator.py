...
class QuestionBankGenerator:
    ...
    def generate_question_bank(
        ...
        questions = self._generate_questions(
            role_competencies, question_types, level_key, num_questions
        )
        ...
        calibration_examples = self._create_calibration_examples(
            questions[:5]
        )  # Sample for first 5 questions
    ...
    def _get_role_competencies(
        self, role_key: str, level_key: str, custom_competencies: list[str] | None
    ) -> list[str]:
        ...
        if custom_competencies:
            competencies.extend(
                [comp.strip() for comp in custom_competencies if comp.strip() not in competencies]
            )

        return list(set(competencies))  # Remove duplicates

    def _generate_questions(
        self, competencies: list[str], question_types: list[str], level: str, num_questions: int
    ) -> list[dict[str, Any]]:
        ...
        for competency in competencies:
            ...
            if "situational" in question_types:
                situational_q = self._generate_situational_questions(
                    competency, questions_per_competency
                )
    ...
    def _create_calibration_examples(
...
def format_human_readable(question_bank: dict[str, Any]) -> str:
    ...
    # Header
    output.append(
        f"Interview Question Bank: {question_bank['role']} ({question_bank['level'].title()} Level)"
    )
    output.append("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate comprehensive interview question banks with scoring criteria"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="senior",
        help="Experience level (junior, mid, senior, staff, principal)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--question-types",
        type=str,
        help="Comma-separated list of question types (technical, behavioral, situational)",
    )
    parser.add_argument(
        "--num-questions", type=int, default=20, help="Number of questions to generate"
    )
