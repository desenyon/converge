...
class InterviewLoopDesigner:
    ...
    def generate_interview_loop(
        self, role: str, level: str, team: str | None = None, competencies: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        rounds = self._design_rounds(
            role_key, level_key, competency_req, role_template, competencies
        )
        ...
        return {
            ...
            "total_duration_minutes": sum(
                round_info["duration_minutes"] for round_info in rounds.values()
            ),
    ...
    def _design_rounds(
        ...
        competency_req: dict,
        role_template: dict,
        custom_competencies: list[str] | None,
    ) -> dict[str, dict]:
        ...
        if level_key in ["senior", "staff", "principal"]:
            if "technical_leadership" in optional_rounds and role_key in [
                "software_engineer",
                "engineering_manager",
            ]:
                core_rounds.append("technical_leadership")
            if "strategic_thinking" in optional_rounds and role_key in [
                "product_manager",
                "engineering_manager",
            ]:
        ...
        for i, round_type in enumerate(core_rounds, 1):
            if round_type in round_definitions:
                ...
                round_def["order"] = i
                round_def["focus_areas"] = self._customize_focus_areas(
                    round_type, competency_req, custom_competencies
                )
    ...
    def _get_round_definitions(self) -> dict[str, dict]:
        ...
        return {
            ...
            "product_sense": {
                ...
                "format": "case_study",
                "objectives": [
                    "Assess product intuition",
                    "Evaluate user empathy",
                    "Review market understanding",
                ],
                "question_types": [
                    "product_scenarios",
            ...
            "design_challenge": {
                ...
                "objectives": [
                    ...
                    "Evaluate user-centered thinking",
                    "Review iteration approach",
                ],
    ...
    def _customize_focus_areas(
        self, round_type: str, competency_req: dict, custom_competencies: list[str] | None
    ) -> list[str]:
    ...
    def _generate_scorecard(
        self, role_key: str, level_key: str, competency_req: dict
    ) -> dict[str, Any]:
    ...
    def _generate_logistics_notes(self, rounds: list[tuple[str, dict]]) -> list[str]:
        ...
        if "hands_on_design" in formats_used:
            notes.append(
                "Provide design tools access or ensure candidate can screen share their preferred tools"
            )
...
def format_human_readable(loop_data: dict[str, Any]) -> str:
    ...
    # Header
    output.append(
        f"Interview Loop Design for {loop_data['role']} ({loop_data['level'].title()} Level)"
    )
    output.append("=" * 60)
    ...
    output.append(
        f"Total Duration: {loop_data['total_duration_minutes']} minutes ({loop_data['total_duration_minutes'] // 60}h {loop_data['total_duration_minutes'] % 60}m)"
    )
    ...
    for day_name, day_info in schedule["day_structure"].items():
        ...
        for item in day_info["rounds"]:
            if item["type"] == "interview":
                output.append(
                    f"  {item['start_time']}-{item['end_time']}: {item['title']} ({item['duration_minutes']}min)"
                )
            else:
                output.append(
                    f"  {item['start_time']}-{item['end_time']}: {item['type'].title()} ({item['duration_minutes']}min)"
                )
    ...
    for round_name, requirements in loop_data["interviewer_requirements"].items():
        ...
        output.append(
            f"Suggested Interviewers: {', '.join(requirements['suggested_interviewers'])}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate calibrated interview loops for specific roles and levels"
    )
    parser.add_argument(
        "--role", type=str, help="Job role title (e.g., 'Senior Software Engineer')"
    )
    parser.add_argument(
        "--level", type=str, help="Experience level (junior, mid, senior, staff, principal)"
    )
    parser.add_argument("--team", type=str, help="Team or department (optional)")
    parser.add_argument(
        "--competencies", type=str, help="Comma-separated list of specific competencies to focus on"
    )
    ...
    try:
        ...
        if args.output:
            ...
            if os.path.isdir(output_path):
                safe_role = "".join(
                    c for c in role.lower() if c.isalnum() or c in (" ", "-", "_")
                ).replace(" ", "_")
