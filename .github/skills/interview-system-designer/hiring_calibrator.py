...
class HiringCalibrator:
    ...
    def analyze_hiring_calibration(
        self,
        ...
        if analysis_type in ["comprehensive", "bias"]:
            analysis_results["bias_analysis"] = self._analyze_bias_patterns(
                processed_data, competencies
            )

        if analysis_type in ["comprehensive", "calibration"]:
            analysis_results["calibration_analysis"] = self._analyze_calibration_consistency(
                processed_data, competencies
            )

        if analysis_type in ["comprehensive", "interviewer"]:
            analysis_results["interviewer_analysis"] = self._analyze_interviewer_bias(
                processed_data
            )

        if analysis_type in ["comprehensive", "scoring"]:
            analysis_results["scoring_analysis"] = self._analyze_scoring_patterns(
                processed_data, competencies
            )

        if trend_analysis:
            analysis_results["trend_analysis"] = self._analyze_trends_over_time(
        ...
        analysis_results["calibration_health_score"] = self._calculate_health_score(
            analysis_results
        )
    ...
    def _generate_data_summary(self, data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        return {
            ...
            "score_statistics": {
                ...
                "std_individual_scores": round(
                    statistics.stdev(all_scores) if len(all_scores) > 1 else 0, 2
                ),
                "mean_average_scores": round(statistics.mean(all_average_scores), 2),
                "std_average_scores": round(
                    statistics.stdev(all_average_scores) if len(all_average_scores) > 1 else 0, 2
                ),
            },
    ...
    def _analyze_bias_patterns(
        self, data: list[dict[str, Any]], target_competencies: list[str] | None
    ) -> dict[str, Any]:
        ...
        if target_competencies:
            bias_analysis["competency_bias"] = self._analyze_competency_bias(
                data, target_competencies
            )
    ...
    def _analyze_demographic_bias(
        self, data: list[dict[str, Any]], demographic: str
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "recommendation": self._generate_demographic_bias_recommendation(
                demographic, bias_details
            )
            if bias_detected
            else None,
        }
    ...
    def _analyze_interviewer_bias(self, data: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(interviewer_analysis) > 1:
            overall_mean_score = statistics.mean(
                [stats["mean_score"] for stats in interviewer_analysis.values()]
            )
            overall_hire_rate = statistics.mean(
                [stats["hire_rate"] for stats in interviewer_analysis.values()]
            )
            ...
            for interviewer_id, stats in interviewer_analysis.items():
                ...
                if stats["mean_score"] > overall_mean_score * (
                    1 + self.bias_thresholds["score_inflation_threshold"]
                ):
                    issues.append("score_inflation")
                elif stats["mean_score"] < overall_mean_score * (
                    1 - self.bias_thresholds["score_deflation_threshold"]
                ):
                ...
                if (
                    ...
                    < self.bias_thresholds["interviewer_consistency_threshold"]
                ):
        ...
        return {
            ...
            "recommendations": self._generate_interviewer_recommendations(
                outlier_interviewers if len(interviewer_analysis) > 1 else {}
            ),
        }

    def _analyze_competency_bias(
        self, data: list[dict[str, Any]], competencies: list[str]
    ) -> dict[str, Any]:
        ...
        for competency in competencies:
            ...
            if len(interviewer_competency_scores) > 1:
                interviewer_means = {
                    interviewer: statistics.mean(scores)
    ...
    def _analyze_calibration_consistency(
        self, data: list[dict[str, Any]], target_competencies: list[str] | None
    ) -> dict[str, Any]:
        ...
        multi_interviewer_candidates = {
            candidate: records
            for candidate, records in candidate_interviewers.items()
            if len(records) > 1
        }
        ...
        agreement_rate = (
            sum(1 for corr in score_correlations if corr["agreement_within_one"])
            / len(score_correlations)
        ...
        if (
            mean_score_std
            > self.calibration_standards["interviewer_agreement"]["maximum_std_deviation"]
        ):
            calibration_quality = "poor"
        elif (
            agreement_rate
            < self.calibration_standards["interviewer_agreement"]["agreement_threshold"]
        ):
    ...
    def _analyze_scoring_patterns(
        ...
        score_percentages = {
            score: count / total_scores for score, count in score_distribution.items()
        }
        ...
        return {
            ...
            "overall_assessment": self._assess_scoring_health(
                distribution_analysis, mean_score, target_mean
            ),
        }
    ...
    def _detect_score_inflation(self, scores: list[float]) -> dict[str, Any]:
        ...
        return {
            ...
            "inflation_detected": deviation > 0.3
            or high_scores_low_variance
            or four_count_ratio > 0.4,
        }
    ...
    def _calculate_overall_interviewer_consistency(
        ...
        return {
            ...
            "std_consistency": round(
                statistics.stdev(interviewer_consistency_scores)
                if len(interviewer_consistency_scores) > 1
                else 0,
                3,
            ),
    ...
    def _identify_improvement_priorities(
        self, health_factors: list[tuple[str, float, float]]
    ) -> list[str]:
        ...
        priorities.sort(
            key=lambda name: next(
                (1 - score) * weight for n, score, weight in health_factors if n == name
            ),
            reverse=True,
        )
                ...
                recommendations.append(
                    {
                        ...
                        "description": demo_analysis.get(
                            "recommendation", f"Implement bias mitigation strategies for {demo}"
                        ),
                        "actions": [
    ...
    def _generate_demographic_bias_recommendation(
        self, demographic: str, bias_details: dict[str, Any]
    ) -> str:
    ...
    def _generate_interviewer_recommendations(
        self, outlier_interviewers: dict[str, Any]
    ) -> list[str]:
        ...
        for interviewer, info in outlier_interviewers.items():
            ...
            if len(issues) >= 2:
                recommendations.append(
                    f"Interviewer {interviewer}: Requires comprehensive recalibration - multiple issues detected"
                )
            elif "score_inflation" in issues:
                recommendations.append(
                    f"Interviewer {interviewer}: Provide calibration training on scoring standards"
                )
            elif "hire_rate_deviation" in issues:
                recommendations.append(
                    f"Interviewer {interviewer}: Review hiring bar standards and decision criteria"
                )

        return recommendations

    def _generate_calibration_recommendations(
        self, mean_std: float, agreement_rate: float
    ) -> list[str]:
        ...
        if mean_std > self.calibration_standards["interviewer_agreement"]["maximum_std_deviation"]:
            recommendations.append(
                "High score variance detected - implement regular calibration sessions"
            )
            recommendations.append(
                "Create shared examples of scoring standards for each competency"
            )
        ...
        if (
            ...
            recommendations.append(
                "Low interviewer agreement rate - standardize interview questions and evaluation criteria"
            )
            recommendations.append(
                "Implement mandatory interviewer training on consistent evaluation"
            )
    ...
    def _assess_scoring_health(
        self, distribution: dict[str, Any], mean_score: float, target_mean: float
    ) -> str:
    ...
    def _generate_trend_insights(
        self, score_trend: float, hire_rate_trend: float, period_metrics: dict[str, Any]
    ) -> list[str]:
        ...
        if hire_rate_variance > 0.01:  # High variance in hire rates
            insights.append(
                "High variance in hire rates across periods - consider process standardization"
            )
...
def format_human_readable(calibration_report: dict[str, Any]) -> str:
    ...
    if bias_analysis:
        ...
        if demographic_bias:
            ...
            for demo, analysis in demographic_bias.items():
                output.append(
                    f"  • {demo.replace('_', ' ').title()}: {analysis.get('bias_details', {}).keys()}"
                )
    ...
    if calibration_analysis and "error" not in calibration_analysis:
        ...
        output.append("-" * 30)
        output.append(
            f"Quality: {calibration_analysis.get('calibration_quality', 'Unknown').title()}"
        )
        output.append(
            f"Agreement Rate: {calibration_analysis.get('agreement_within_one_point_rate', 0):.1%}"
        )
        output.append(
            f"Score Std Dev: {calibration_analysis.get('mean_score_standard_deviation', 0):.3f}"
        )
    ...
    if scoring_analysis:
        ...
        output.append("-" * 30)
        output.append(
            f"Overall Assessment: {scoring_analysis.get('overall_assessment', 'Unknown').title()}"
        )

        score_stats = scoring_analysis.get("score_statistics", {})
        output.append(
            f"Mean Score: {score_stats.get('mean_score', 0):.2f} (Target: {score_stats.get('target_mean', 0):.2f})"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze interview data for bias and calibration issues"
    )
    parser.add_argument(
        "--input", type=str, required=True, help="Input JSON file with interview results data"
    )
    parser.add_argument(
        "--analysis-type",
        type=str,
        choices=["comprehensive", "bias", "calibration", "interviewer", "scoring"],
        default="comprehensive",
        help="Type of analysis to perform",
    )
    parser.add_argument(
        "--competencies", type=str, help="Comma-separated list of competencies to focus on"
    )
    parser.add_argument(
        "--trend-analysis", action="store_true", help="Perform trend analysis over time"
    )
    parser.add_argument(
        "--period",
        type=str,
    ...
    parser.add_argument(
        "--format", choices=["json", "text", "both"], default="both", help="Output format"
    )
    ...
    try:
        ...
        if "error" in results:
            ...
            print(
                f"Health Score: {health_score.get('overall_score', 0):.3f} ({health_score.get('health_category', 'Unknown').title()})"
            )
            ...
            if recommendations:
                print(
                    f"Top Priority: {recommendations[0]['title']} ({recommendations[0]['priority'].title()})"
                )
