...
class Story:
    ...
    def __init__(self, data: dict[str, Any]):
        ...
        self.completed_date: str | None = data.get("completed_date")
...
def _generate_detailed_metrics(sprints: list[SprintHealthData]) -> dict[str, Any]:
    ...
    if all_blockers:
        resolution_times = [
            b.get("resolution_days", 0) for b in all_blockers if b.get("resolution_days", 0) > 0
        ]
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Analyze sprint health across multiple dimensions")
