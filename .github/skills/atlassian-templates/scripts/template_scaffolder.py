...
def template_meeting_notes() -> dict[str, Any]:
    ...
    body += _section(
        "Agenda", "<ol><li><p>Item 1</p></li><li><p>Item 2</p></li><li><p>Item 3</p></li></ol>"
    )
...
def template_how_to_guide() -> dict[str, Any]:
    ...
    body += _section(
        "Overview", "<p>Brief description of what this guide covers and who it is for.</p>"
    )
    body += _section(
        "Prerequisites", "<ul><li><p>Prerequisite 1</p></li><li><p>Prerequisite 2</p></li></ul>"
    )
    body += _section(
    ...
    body += _section(
        "Troubleshooting",
        macro_expand(
            "Common Issues", "<h3>Issue 1</h3><p>Solution...</p><h3>Issue 2</h3><p>Solution...</p>"
        ),
    )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate Confluence page template markup")
    parser.add_argument(
        "template",
        nargs="?",
