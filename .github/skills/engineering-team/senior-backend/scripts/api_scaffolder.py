...
class APIScaffolder:
    ...
    def generate_route_file(self, tag: str, operations: list[dict]):
        ...
        for op in operations:
            ...
            lines.append(
                f"router.{method}('{path}', {middleware}async (req: Request, res: Response, next: NextFunction) => {{"
            )
            lines.append("  try {")
...
Examples:
  ...
  %(prog)s openapi.yaml --types-only --output src/types/
    ...
    parser.add_argument("spec", help="Path to OpenAPI specification (YAML or JSON)")
    parser.add_argument(
        "--output", "-o", default="./generated", help="Output directory (default: ./generated)"
    )
    parser.add_argument(
        "--framework",
        "-f",
    ...
    parser.add_argument("--types-only", action="store_true", help="Generate only TypeScript types")
