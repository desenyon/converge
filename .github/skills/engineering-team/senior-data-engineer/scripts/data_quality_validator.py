...
class TypeDetector:
    ...
    @classmethod
    def detect_pattern(cls, values: list[str]) -> str | None:
...
class BaseValidator(ABC):
    ...
    @abstractmethod
    def validate(
        self, data: list[dict], schema: DataSchema | None = None
...
class SchemaValidator(BaseValidator):
    ...
    def _validate_column(
        self, data: list[dict], col_schema: ColumnSchema
    ) -> list[ValidationResult]:
        results = []
    ...
    def _validate_primary_key(
        self, data: list[dict], pk_columns: list[str]
    ) -> list[ValidationResult]:
        """Validate primary key uniqueness"""
        results = []
        pk_values = []
...
class DataProfiler:
    ...
    def profile(self, data: list[dict], name: str = "dataset") -> DataProfile:
        ...
        memory_size = sys.getsizeof(data) + sum(
            sys.getsizeof(row) + sum(sys.getsizeof(v) for v in row.values()) for row in data
        )
...
class GreatExpectationsGenerator:
        ...
        expectations.append(
            {
                ...
                "kwargs": {"value": profile.column_count},
            }
        )
    ...
    def _generate_column_expectations(self, col_profile: ColumnProfile) -> list[dict]:
        ...
        # Column exists
        expectations.append(
            {"expectation_type": "expect_column_to_exist", "kwargs": {"column": col_name}}
        )
            ...
            expectations.append(
                {
                    ...
                    "kwargs": {"column": col_name, "strftime_format": "%Y-%m-%d"},
                }
            )
        ...
        if col_profile.unique_count <= 20 and col_profile.unique_percentage < 10:
            top_values = [
                v[0] for v in col_profile.top_values if v[1] > col_profile.total_count * 0.01
            ]
            if top_values:
                expectations.append(
                    {
                        "expectation_type": "expect_column_values_to_be_in_set",
                        "kwargs": {"column": col_name, "value_set": top_values, "mostly": 0.95},
                    }
                )
...
class QualityScoreCalculator:
    ...
    def calculate(
        self, profile: DataProfile, validation_results: list[ValidationResult]
    ) -> QualityScore:
        """Calculate quality score from profile and validation results"""
        # Completeness: average non-null percentage
        completeness = (
            100 - statistics.mean([c.null_percentage for c in profile.columns])
            if profile.columns
            else 0
        )
        ...
        uniqueness = (
            statistics.mean([c.unique_percentage for c in unique_cols]) if unique_cols else 100
        )
        ...
        consistency = (
            ((total_checks - error_checks) / total_checks * 100) if total_checks > 0 else 100
        )
        ...
        accuracy = (
            min(100, 50 + (pattern_detected / len(profile.columns) * 50)) if profile.columns else 50
        )
...
class ReportGenerator:
    ...
    def generate_text_report(
        self, profile: DataProfile, results: list[ValidationResult], score: QualityScore
    ) -> str:
        ...
        lines.append("-" * 40)
        lines.append(
            f"  Overall:      {score.overall:>6.1f}% {'✓' if score.overall >= 80 else '✗'}"
        )
        ...
        for col in profile.columns:
            ...
            if col.min_length is not None:
                lines.append(
                    f"    Length: [{col.min_length}, {col.max_length}] (avg: {col.avg_length:.1f})"
                )
    ...
    if args.json or args.output:
        ...
        for col in profile.columns:
            ...
            if col.mean is not None:
                lines.append(
                    f"    Stats: min={col.min_value}, max={col.max_value}, mean={col.mean:.2f}"
                )
...
def cmd_schema(args):
    ...
    # Generate schema
    schema = {"name": profile.name, "version": "1.0", "columns": []}
...
Examples:
  ...
  python data_quality_validator.py schema data.csv --output schema.json
    ...
    validate_parser.add_argument(
        "--detect-anomalies", action="store_true", help="Detect statistical anomalies"
    )
    ...
    contract_parser.add_argument(
        "--contract", "-c", required=True, help="Data contract file (YAML or JSON)"
    )
