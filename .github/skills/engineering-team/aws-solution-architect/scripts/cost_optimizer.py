...
class CostOptimizer:
    ...
    def analyze_and_optimize(self) -> dict[str, Any]:
        ...
        return {
            ...
            "savings_percentage": round((potential_savings / self.monthly_spend) * 100, 2)
    ...
    def _analyze_compute(self) -> float:
        ...
        if ec2_instances:
            ...
            on_demand_count = sum(
                1 for inst in ec2_instances if inst.get("pricing", "on-demand") == "on-demand"
            )
        ...
        if lambda_functions:
            oversized = sum(
                ...
                if fn.get("memory_mb", 128) > 512 and fn.get("avg_memory_used_mb", 0) < 256
            )
            if oversized > 0:
    ...
    def _analyze_database(self) -> float:
        ...
        for table in dynamodb_tables:
            if table.get("billing_mode", "PROVISIONED") == "PROVISIONED":
                ...
                if utilization < 20:
                    on_demand_savings = (
                        (read_capacity * 0.00013 + write_capacity * 0.00065) * 730 * 0.3
                    )
