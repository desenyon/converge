...
class SecurityScanner:
    ...
    SECRET_PATTERNS = [
        ...
        (r"sk-[a-zA-Z0-9]{48}", "OpenAI API Key", "OpenAI API key detected"),
        (
    ...
    SQL_INJECTION_PATTERNS = [
        (r'execute\s*\(\s*["\']?\s*SELECT.*\+.*\+', "Dynamic SQL query with string concatenation"),
        (r'execute\s*\(\s*f["\']SELECT', "F-string SQL query (Python)"),
        (r'cursor\.execute\s*\(\s*["\'].*%s.*%\s*\(', "Unsafe string formatting in SQL"),
        (r'query\s*\(\s*[`"\']SELECT.*\$\{', "Template literal SQL injection (JavaScript)"),
        (r'\.query\s*\(\s*["\'].*\+.*\+', "String concatenation in SQL query"),
    ]

    # XSS patterns
    XSS_PATTERNS = [
        (r"innerHTML\s*=\s*[^;]+(?:user|input|param|query)", "User input assigned to innerHTML"),
        (r"document\.write\s*\([^;]*(?:user|input|param|query)", "User input in document.write"),
        (r"\.html\s*\(\s*[^)]*(?:user|input|param|query)", "User input in jQuery .html()"),
        (r"dangerouslySetInnerHTML", "React dangerouslySetInnerHTML usage"),
        (r"\|safe\s*}}", "Django safe filter may disable escaping"),
    ]
    ...
    COMMAND_INJECTION_PATTERNS = [
        ...
        (r"exec\s*\(\s*[^)]*(?:user|input|param|request)", "exec() with potential user input"),
        (r"eval\s*\(\s*[^)]*(?:user|input|param|request)", "eval() with potential user input"),
    ]

    # Path traversal patterns
    PATH_TRAVERSAL_PATTERNS = [
        (r"open\s*\(\s*[^)]*(?:user|input|param|request)", "File open with potential user input"),
        (
    ...
    def __init__(self, target_path: str, severity_threshold: str = "low", verbose: bool = False):
        """
    ...
    def scan(self) -> dict:
        ...
        filtered_findings = [
            f for f in self.findings if self.severity_order.get(f.severity, 3) <= threshold_level
        ]
        ...
        for line_num, line in enumerate(lines, 1):
            for pattern_tuple in patterns:
                ...
                if match:
                    ...
                    severity = self._calculate_severity(default_severity, file_path, category)
...
Examples:
  ...
  %(prog)s /path/to/file.py --verbose
    ...
    scanner = SecurityScanner(
        target_path=args.target, severity_threshold=args.severity, verbose=args.verbose
    )
