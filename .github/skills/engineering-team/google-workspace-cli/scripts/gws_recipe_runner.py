...
RECIPES: dict[str, Recipe] = {
    ...
    "send-email": Recipe(
        ...
        "email",
        ["gmail"],
        ["gws gmail users.messages send me --to {to} --subject {subject} --body {body}"],
    ),
    "reply-to-thread": Recipe(
        "reply-to-thread",
        "Reply to an existing email thread",
        "email",
        ["gmail"],
        ["gws gmail users.messages reply me --thread-id {thread_id} --body {body}"],
    ),
    "forward-email": Recipe(
        "forward-email",
        "Forward an email to another recipient",
        "email",
        ["gmail"],
        ["gws gmail users.messages forward me --message-id {msg_id} --to {to}"],
    ),
    "search-emails": Recipe(
        "search-emails",
    ...
    "label-manager": Recipe(
        "label-manager",
        "Create, list, and organize Gmail labels",
        "email",
        ["gmail"],
        ["gws gmail users.labels list me --json", "gws gmail users.labels create me --name {name}"],
    ),
    "filter-setup": Recipe(
        "filter-setup",
        "Create email filters for auto-labeling",
        "email",
        ["gmail"],
        ["gws gmail users.settings.filters create me --criteria {criteria} --action {action}"],
    ),
    "unread-digest": Recipe(
        "unread-digest",
    ...
    "upload-file": Recipe(
        ...
        "files",
        ["drive"],
        ["gws drive files create --name {name} --upload {path} --parents {folder_id}"],
    ),
    "create-sheet": Recipe(
        "create-sheet",
    ...
    "share-file": Recipe(
        ...
        "files",
        ["drive"],
        ["gws drive permissions create {file_id} --type user --role writer --emailAddress {email}"],
    ),
    "export-file": Recipe(
        "export-file",
        ...
        "files",
        ["drive"],
        ["gws drive files export {file_id} --mime application/pdf --output {output}"],
    ),
    "list-files": Recipe(
        "list-files",
    ...
    "cleanup-trash": Recipe(
        "cleanup-trash", "Empty Drive trash", "files", ["drive"], ["gws drive files emptyTrash"]
    ),
    # Calendar (6)
    "create-event": Recipe(
        "create-event",
    ...
    "find-time": Recipe(
        ...
        ["calendar"],
        [
            "gws helpers find-time --attendees {attendees} --duration {minutes} --within {date_range}"
        ],
    ),
    "today-schedule": Recipe(
        "today-schedule",
    ...
    "meeting-prep": Recipe(
        "meeting-prep",
        "Prepare for an upcoming meeting (agenda + attendees)",
        "calendar",
        ["calendar"],
    ...
    "reschedule": Recipe(
        ...
        "calendar",
        ["calendar"],
        ["gws calendar events patch primary {event_id} --start {new_start} --end {new_end}"],
    ),
    # Reporting (5)
    "standup-report": Recipe(
        "standup-report",
    ...
    "share-folder": Recipe(
        ...
        ["drive"],
        [
            "gws drive permissions create {folder_id} --type group --role writer --emailAddress {group}"
        ],
    ),
    "create-doc": Recipe(
        "create-doc",
    ...
    "chat-message": Recipe(
        ...
        "collaboration",
        ["chat"],
        ["gws chat spaces.messages create {space} --text {message}"],
    ),
    "list-spaces": Recipe(
        "list-spaces",
    ...
    "task-create": Recipe(
        ...
        "collaboration",
        ["tasks"],
        ["gws tasks tasks insert {tasklist_id} --title {title} --due {due_date}"],
    ),
    # Data (4)
    "sheet-read": Recipe(
        "sheet-read",
        ...
        "data",
        ["sheets"],
        ["gws sheets spreadsheets.values get {sheet_id} --range {range} --json"],
    ),
    "sheet-write": Recipe(
        "sheet-write",
        ...
        "data",
        ["sheets"],
        ["gws sheets spreadsheets.values update {sheet_id} --range {range} --values {data}"],
    ),
    "sheet-append": Recipe(
        "sheet-append",
        ...
        "data",
        ["sheets"],
        ["gws sheets spreadsheets.values append {sheet_id} --range {range} --values {data}"],
    ),
    "export-contacts": Recipe(
        "export-contacts",
    ...
    "eod-wrap": Recipe(
        "eod-wrap",
        "End-of-day wrap up: summarize completed, pending, tomorrow",
        "cross-service",
        ["calendar", "tasks"],
        [
            "gws calendar events list primary --timeMin {today_start} --timeMax {today_end} --json",
    ...
    "inbox-zero": Recipe(
        "inbox-zero",
        "Process inbox to zero: label, archive, reply, task",
        "cross-service",
        ["gmail", "tasks"],
        [
...
def list_recipes(persona: str | None, output_json: bool):
...
def describe_recipe(name: str, output_json: bool):
    ...
    if not recipe:
        ...
        print("Use --list to see available recipes")
        sys.exit(1)
