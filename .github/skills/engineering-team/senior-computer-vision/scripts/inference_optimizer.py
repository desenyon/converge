...
class InferenceOptimizer:
    ...
    def benchmark(
        ...
        try:
            if self.model_format == "onnx":
                results.update(
                    self._benchmark_onnx(input_size, batch_sizes, num_iterations, warmup)
                )
            elif self.model_format == "pytorch":
                results.update(
                    self._benchmark_pytorch(input_size, batch_sizes, num_iterations, warmup)
                )
            else:
    ...
    def _benchmark_onnx(
        self, input_size: tuple[int, int], batch_sizes: list[int], num_iterations: int, warmup: int
    ) -> dict[str, Any]:
...
def main():
    ...
    parser.add_argument(
        "--batch-sizes", type=int, nargs="+", default=[1, 4, 8], help="Batch sizes to benchmark"
    )
    parser.add_argument(
