    ...
    def _voc_to_coco(self) -> dict:
        ...
        for img_id, xml_file in enumerate(self.input_path.rglob("*.xml"), 1):
            try:
                ...
                coco_data["images"].append(
                    {"id": img_id, "file_name": filename.text, "width": width, "height": height}
                )
                results["converted_images"] += 1
                ...
                for obj in root.findall("object"):
                    ...
                    if name not in class_to_id:
                        ...
                        coco_data["categories"].append(
                            {"id": class_to_id[name], "name": name, "supercategory": "object"}
                        )
    ...
    def _to_albumentations(config: dict, task: str) -> dict:
        ...
        for aug_name, params in config.items():
            if aug_name == "horizontal_flip":
                ...
                transforms.append(
                    {"type": "Rotate", "limit": params.get("limit", 15), "p": params.get("p", 0.5)}
                )
            elif aug_name == "scale":
                transforms.append(
                    {
...
Examples:
  ...
  Validate dataset:
    ...
    convert_parser.add_argument(
        "--format", "-f", required=True, choices=["yolo", "coco", "voc"], help="Target format"
    )
    convert_parser.add_argument(
        "--source-format",
        "-s",
